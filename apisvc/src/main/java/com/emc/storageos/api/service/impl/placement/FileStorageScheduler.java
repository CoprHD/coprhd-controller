...
public class FileStorageScheduler {
    ...
    private List<FileRecommendation> getRecommendedStoragePortsForVNAS(
		...
		List<VirtualNAS> vNASList = getVNASServersInProject(project, vArrayURI, vPool);
		...
		for (Recommendation recommendation : poolRecommendations) {
			...
			if (vNAS != null && rec.getSourceDevice().equals(vNAS.getStorageDeviceURI())) {

				_log.debug("Getting the storage ports of VNAS server: {}", vNAS.getId());
	...
	private List<VirtualNAS> getUnassignedVNASServers(URI vArrayURI, VirtualPool vpool) {
		...
		if(vNASList != null && !vNASList.isEmpty()) {
			for (Iterator<VirtualNAS> iterator = vNASList.iterator(); iterator.hasNext();) {
				...
				if(vNAS.getProject() != null) {
					...
					iterator.remove();
				} else if(Boolean.valueOf(vNAS.getMetrics().get("isOverloaded"))) {
					iterator.remove();
		...
		if(spList!=null && !spList.isEmpty()) {
			Collections.sort(spList, new Comparator<StoragePort>() {
				...
				public int compare(StoragePort sp1, StoragePort sp2) {
					final String PORT_USED_PERCENTAGE = "avgPercentBusy";
					if (sp1.getMetrics() != null && sp2.getMetrics() != null) {
	
						String sp1UsedPercent = sp1.getMetrics().get(
								PORT_USED_PERCENTAGE);
						String sp2UsedPercent = sp2.getMetrics().get(
								PORT_USED_PERCENTAGE);
	
						if (sp1UsedPercent != null && sp2UsedPercent != null) {
							return Double.valueOf(sp1UsedPercent).compareTo(
									Double.valueOf(sp2UsedPercent));
						}
					}
					return 0;
				}
	...
	private VirtualNAS getTheLeastUsedVNASServer(List<VirtualNAS> vNASList,
		...
		Collections.sort(vNASList, new Comparator<VirtualNAS>() {
			...
			public int compare(VirtualNAS v1, VirtualNAS v2) {
				...
				Double avgPercentUsedV1 = Double.valueOf(v1.getMetrics().get(
						AVG_PERCENATAGE_USED_PARAM));
				Double avgPercentUsedV2 = Double.valueOf(v2.getMetrics().get(
						AVG_PERCENATAGE_USED_PARAM));
	...
	private List<VirtualNAS> getVNASServersInProject(Project project, URI varrayUri, VirtualPool vpool) {
		...
		if (vNASServerIdSet != null && !vNASServerIdSet.isEmpty()) {
			...
			for (Iterator<VirtualNAS> iterator = vNASList.iterator(); iterator
				...
				if (!isVNASActive(virtualNAS)) {
					iterator.remove();
				} else if(!virtualNAS.getTaggedVirtualArrays().contains(varrayUri)) {
					iterator.remove();
				} else if(Boolean.valueOf(virtualNAS.getMetrics().get("isOverloaded"))) {
					iterator.remove();
				} else if(!virtualNAS.getProtocols().containsAll(vpool.getProtocols())) {
					iterator.remove();
				}
			}
	...
	private boolean isVNASActive(VirtualNAS virtualNAS) {
		...
		if (virtualNAS.getInactive()
				...
				|| !DiscoveredDataObject.CompatibilityStatus.COMPATIBLE
						.name().equals(
								virtualNAS.getCompatibilityStatus())
				|| !vNasState.LOADED.name().equals(
						virtualNAS.getVNasState())
				|| !DiscoveryStatus.VISIBLE.name().equals(
