...
public class ClientHttpMethodsImpl implements ClientHttpMethods, InitializingBean, ApplicationContextAware {
    ...
    private Authentication endpointLogin(String serviceURI, String username, String password) throws ClientGeneralException {
        ...
        com.emc.cloud.platform.ucs.out.model.AaaLogin response = httpRequest.httpPostXMLObject(getServiceURI(),
                objectFactory.createAaaLogin(aaaLogin), com.emc.cloud.platform.ucs.out.model.AaaLogin.class);
        ...
        if (response != null && response.getOutCookie() != null && !response.getOutCookie().isEmpty()) {
            ...
            throw new ClientGeneralException(ClientMessageKeys.UNAUTHORIZED, new String[] { getServiceURI(), "",
    ...
    public <T> T postEntity(JAXBElement<?> jaxbElement, Class<T> returnType) throws ClientGeneralException {
        ...
        try {
            ...
            if (authentication instanceof UsernamePasswordAuthenticationToken && authentication.isAuthenticated()) {
                com.emc.cloud.platform.ucs.out.model.AaaLogin loginResponse = (com.emc.cloud.platform.ucs.out.model.AaaLogin) authentication
                        .getDetails();
                try {
    ...
    private void endpointLogout(Authentication authentication) throws ClientGeneralException {
        ...
        ClientHttpRequest httpRequest = requestFactory.create();
        com.emc.cloud.platform.ucs.out.model.AaaLogout response = httpRequest.httpPostXMLObject(getServiceURI(),
                objectFactory.createAaaLogout(aaaLogout), com.emc.cloud.platform.ucs.out.model.AaaLogout.class);

        if (OutStatus.SUCCESS.getValue().equals(response.getOutStatus())) {
            return;
        } else {
            throw new ClientGeneralException(ClientMessageKeys.SECURITY_EXCEPTION, new String[] { "Unable to log out session : "
