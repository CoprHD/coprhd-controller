...
public class DbHostInterfaceLabelMigrationTest extends DbSimpleMigrationTestBase {
	...
	private void prepareInitiatorData() {
		...
		Assert.assertTrue("Expected " + INSTANCES_TO_CREATE + " prepared " + Initiator.class.getSimpleName() + ", found only " + count, count == INSTANCES_TO_CREATE); 
	}
	...
	private void verifyInitiatorData() {
		...
		Assert.assertTrue("We should still have " + INSTANCES_TO_CREATE + " " + Initiator.class.getSimpleName() + " after migration, not " + count, count == INSTANCES_TO_CREATE);     
	}
