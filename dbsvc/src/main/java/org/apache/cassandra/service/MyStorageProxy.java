...
import com.emc.storageos.db.server.impl.MyIndexSearcher;
import com.emc.storageos.db.server.impl.MyRowKey;
import com.emc.storageos.db.server.impl.MyRowKeys;
import com.emc.storageos.db.server.impl.MyRowMapper;
 ...
 */
public class MyStorageProxy {
    private static final Logger logger = LoggerFactory.getLogger(MyStorageProxy.class);
    ...
    private static MyRowKey rowKey(AbstractBounds<RowPosition> range, MyRowKeys rowKeys) {
    ...
    public static MyRowKey last(MyRowMapper mapper, MyRowKey rowKey, List<Row> rows, List<Row> processedRows) {
    ...
    public static Pair<List<Row>, MyRowKeys> getRangeSlice(MyIndexSearcher searcher,
                                                         String keyspaceName,
                                                         String columnFamily,
                                                         long timestamp,
        ...
        logger.info("lbymm0 rowKeys={}", rowKeys);
        ...
        try {
            ...
            logger.info("lbymm0 ranges.size={}", ranges.size());
            ...
            logger.info("lbydd ranges size={}", ranges.size());
            while (i < ranges.size()) {
                ...
                while ((i - concurrentFetchStartingIndex) < concurrencyFactor) {
                    ...
                    ////////////////////////////////////////////////////////////////////////////////////////////////////
                    // MyRowKey after = rowKey(range, rowKeys);

                    ByteBuffer bb = ByteBuffer.allocate(4);
                    bb.putInt(startPos);

                    List<IndexExpression> decoratedExpressions = new ArrayList<>(expressions);
                    //logger.info("lbymm after={}", after);
                    logger.info("lbymm startPos={}", startPos);
                    logger.info("lbymm liveEndpoints={} filterEndpoints={}", liveEndpoints, filteredEndpoints);
                    //if (after != null) {
                    if (startPos != 0) {
                        decoratedExpressions.add(new IndexExpression(MyIndexSearcher.AFTER,
                                Operator.EQ,
                    ...
                    if (filteredEndpoints.size() == 1 &&
                        ...
                        StageManager.getStage(Stage.READ)
                                .execute(new StorageProxy.LocalRangeSliceRunnable(nodeCmd, handler), Tracing.instance.get());
                    } else {
                        MessageOut<? extends AbstractRangeCommand> message = nodeCmd.createMessage();
                        logger.info("lbymm0 send read command to {}", filteredEndpoints);
    ...
    public static Pair<List<Row>, MyRowKeys> makeResult(List<Row> rows,
        ...
        MyRowKeys newRowKeys = new MyRowKeys();
        for (Map.Entry<AbstractBounds<RowPosition>, List<Row>> entry : rowsPerRange.entrySet()) {
            MyRowKey rowKey = rowKey(entry.getKey(), rowKeys);
            MyRowKey newRowKey = last(mapper, rowKey, rows, entry.getValue());
