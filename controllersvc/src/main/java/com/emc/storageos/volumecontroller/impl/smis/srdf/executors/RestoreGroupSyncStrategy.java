 ...
 */
public class RestoreGroupSyncStrategy implements ExecutorStrategy {
    private SmisCommandHelper helper;

    public RestoreGroupSyncStrategy(SmisCommandHelper helper) {
        this.helper = helper;
    }

    @Override
    public void execute(Collection<CIMObjectPath> objectPaths, StorageSystem provider) throws WBEMException {
        CIMArgument[] args = helper.getSRDFRestoreInputArguments(objectPaths.iterator().next());
