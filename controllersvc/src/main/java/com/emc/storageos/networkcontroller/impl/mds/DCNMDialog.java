...
public class DCNMDialog extends BaseSANCIMObject {
	...
	public class FCProtocolEndpoint {
		 String wwpn;
		    ...
		    FCProtocolEndpoint(CIMInstance ins) {
		    	...
		    	if (wwnn.matches(_wwnRegex) == false) wwnn = "";
		    	String ifName = null;
		    ...
		    void findConnections(WBEMClient client, String namespace) {
		        ...
		        while(instances.hasNext()) {
		            ...
		            if (! instance.getProperty("ProtocolIFType").getValue().toString().equals("56")) continue;
	...
	public List<String> getFabricIds() throws Exception {
        ...
        try {
            ...
            while (vsanIt.hasNext()) {
                ...
                if (vsanId != null) fabricIds.add(vsanId);
            }
        } finally {
            if (vsanIt != null) {
                vsanIt.close();
	...
	private List<Zoneset> getZonesetsInVsan(CIMInstance vsanIns) throws WBEMException {
        ...
        List<Zoneset> zonesets = new ArrayList<Zoneset>();
        if (activeZoneset != null) zonesets.add(activeZoneset);
...
//			}
	...
	public List<String> addZonesStrategy(List<Zone> zones, Integer vsanId) 
		...
		if (zonesetServicePath == null) throw new DeviceControllerException("Couldn't locate ZoneSetService vsan: " + vsanId);
		...
		try {
			...
			if (zonesets == null || zonesets.isEmpty()) throw new DeviceControllerException("no zonesets");
			Zoneset activeZoneset = zonesets.get(0);
			if (activeZoneset.getActive() != true) throw new DeviceControllerException("no active zoneset");
