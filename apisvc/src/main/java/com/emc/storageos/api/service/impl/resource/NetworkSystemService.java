...
public class NetworkSystemService extends TaskResourceService {
    ...
    private void checkForDuplicateDevice(URI id, String ip_address, Integer port_number,
    	...
    	for (URI uri : existingDevices) {
        	...
        	if (existing == null || existing.getId().equals(id)) continue;
	...
	public TaskResourceRep deleteNetworkSystem(@PathParam("id") URI id)
        ...
        List<URI> nsystems = null; List<FCZoneReference> zones = null;
    ...
    public NetworkSystemRestRep registerNetworkSystem(@PathParam("id") URI id) throws ControllerException{
        ...
        if( RegistrationStatus.UNREGISTERED.toString().equalsIgnoreCase(
	        ...
	        auditOp(OperationTypeEnum.REGISTER_NETWORK_SYSTEM, true, null,
	                ...
	                networkSystem.getSmisProviderIP(), networkSystem.getSmisPortNumber(), networkSystem.getSmisUserName(), networkSystem.getSmisUseSSL()
	                );
        }
     ...
     * @return A list of the active zones and their members.  If zone name is specified, and there is a match, then only one zone is returned.  
    ...
    @CheckPermission(roles = {Role.SYSTEM_ADMIN})
    public SanZonesRestRep getSanZones(@PathParam("id") URI id, @PathParam("fabricId") String fabricId, @QueryParam("zone-name") String zoneName, @QueryParam("exclude-members") boolean excludeMembers) throws InternalException{
     ...
     * <p>
     * Current address WWN is optional; however, if provided, it must match
     * the one in system before update.  If not, exception will be thrown. 
     * <p>
     * This is an asynchronous call.
