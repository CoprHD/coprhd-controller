...
/**
 * FilePolicyProfile resource implementation
 * 
 * @author lakhiv
 * 
 */
...
public class FilePolicyProfileService extends TaggedResource {
    ...
    private static final Logger _log = LoggerFactory.getLogger(FilePolicyProfileService.class);

    protected static final String EVENT_SERVICE_SOURCE = "FilePolicyProfileService";

    private static final String EVENT_SERVICE_TYPE = "filePolicyProfile";
    ...
    protected ResourceTypeEnum getResourceType() {
        return ResourceTypeEnum.FILE_POLICY_PROFILE;
    }

    @SuppressWarnings("unchecked")
    @Override
    public Class<FilePolicyProfile> getResourceClass() {
        return FilePolicyProfile.class;
    }

    /**
     * Get policy profile object from id
     * 
     * @param id the URN of a CoprHD file policy profile
     * @return
     */
    private FilePolicyProfile getPolicyProfileById(URI id, boolean checkInactive) {
        if (id == null) {
            return null;
        }

        FilePolicyProfile policyProfile = _permissionsHelper.getObjectById(id, FilePolicyProfile.class);
        ArgValidator.checkEntity(policyProfile, id, isIdEmbeddedInURL(id));
        return policyProfile;
    }

    @Override
    protected FilePolicyProfile queryResource(URI id) {
        ArgValidator.checkUri(id);
        return getPolicyProfileById(id, false);
    }

    @Override
    protected URI getTenantOwner(URI id) {
    	FilePolicyProfile policyProfile = queryResource(id);
        return policyProfile.getTenantOrg().getURI();
    }

    /**
     * Get the details of a file policy profile.
     * 
     * @param id the URN of a policy profile.
     * 
     * @brief Show policy profile
     * @return A FilePolicyProfileRestRep reference specifying the data for the
     *         file policy profile
     */
    @GET
    ...
    public FilePolicyProfile getFilePolicyProfile(@PathParam("id") URI id) {
        ArgValidator.checkFieldUriType(id, FilePolicyProfile.class, "id");
        FilePolicyProfile policyProfile = queryResource(id);
        return policyProfile;
    }
    ...
    @Path("/{id}")
    @CheckPermission(roles = { Role.TENANT_ADMIN })
    public Response deletePolicyProfile(@PathParam("id") URI profileId) {
        ArgValidator.checkFieldUriType(profileId, FilePolicyProfile.class, "id");
        FilePolicyProfile policyProfile = getPolicyProfileById(profileId, true);
        ArgValidator.checkReference(FilePolicyProfile.class, profileId, checkForDelete(policyProfile));
        
        _dbClient.markForDeletion(policyProfile);
        _log.info("File policy profile {} deleted successfully", policyProfile.getLabel());
        return Response.ok().build();
    }
