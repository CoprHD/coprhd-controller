 ...
 */
public class EstablishStorageSyncsStrategy implements ExecutorStrategy {

    private SmisCommandHelper helper;

    public EstablishStorageSyncsStrategy(SmisCommandHelper helper) {
        this.helper = helper;
    }

    @Override
    public void execute(Collection<CIMObjectPath> objectPaths, StorageSystem provider) throws WBEMException {
        CIMArgument[] args = helper.getResumeSyncListInputArguments(objectPaths);
