...
public class IsilonApi {
	...
	private static final URI URI_TARGET_REPLICATION_POLICY_REPORTS = URI
			.create("/platform/1/sync/target/reports?policy_name=");
	...
	public IsilonClusterInfo getClusterInfo() throws IsilonException {
		...
		try {
			...
			return new Gson().fromJson(SecurityUtils.sanitizeJsonString(resp.toString()), IsilonClusterInfo.class);
	...
	public IsilonClusterConfig getClusterConfig() throws IsilonException {
		...
		try {
			...
			JSONObject resp = clientResp.getEntity(JSONObject.class);
			return new Gson().fromJson(SecurityUtils.sanitizeJsonString(resp.toString()), IsilonClusterConfig.class);
	...
	public boolean fsDirHasData(String fspath) throws IsilonException {
		...
		try {
			...
			if (clientResp.getStatus() != 200) {
				processErrorResponse("list", "files", clientResp.getStatus(), clientResp.getEntity(JSONObject.class));
			} else {
				JSONObject resp = clientResp.getEntity(JSONObject.class);
			...
			// TODO why we are suppressing exception here??
	 ...
	 *            Dir path to be created
	 * @param recursive
	 *            if true, will create parent recursively if it doesn't exist
	 * @throws IsilonException
	 */
	...
	/**
	 * Delete directory on isilon No recursive flag to Isilon API, So the delete
	 * directory fails if there are any files and folders in it.
	 * 
	 * @param fspath
	 *            directory path
	 * 
	...
	private void deleteSnapshotSchedule(URI url) throws IsilonException {
		...
		try {
			...
			// error 404 means Snapshot Schedule can not be found, assuming it
			// already deleted.
			if (resp.getStatus() != 200 && resp.getStatus() != 204 && resp.getStatus() != 404) {
	 ...
	 * @throws IsilonException
	 */
	public String createSnapshotSchedule(IsilonSnapshotSchedule isiSchedule) throws IsilonException {
		sLogger.info("Isilon snapshot schedule: {} creation started", isiSchedule.toString());
	...
	private <T> void modify(URI url, String id, String key, T obj) throws IsilonException {
		...
		try {
			...
			String response = String.format("%1$s", (resp == null) ? "" : resp);
			throw IsilonException.exceptions.modifyResourceFailedOnIsilonArray(key, id, response, e);
		} finally {
			if (resp != null) {
				resp.close();
			}
	 ...
	 * @param force
	 *            boolean flag to ignore client FQDN check against DNS
	 ...
	 * @param thresholds
	 *            optional long values for the thresholds if none specified, an
	 *            un-enforced quota will be created otherwise, the first value
	 *            is used for hard limit and rest ignored for now
	 ...
	 *            value to indicate if overhead is to be included in the quota
	 * @param bIncludeSnapshots
	 *            value to indicate if snapshot size is to be included in the
	 *            quota
	 * @return Identifier for the quota created
	 * @throws IsilonException
	 */
	public String createQuota(String path, Long fsSize, boolean bThresholdsIncludeOverhead, boolean bIncludeSnapshots,
		...
		if (thresholds != null && thresholds.length > 0
			...
			quota = constructIsilonSmartQuotaObjectWithThreshold(path, "directory", fsSize, bThresholdsIncludeOverhead,
					bIncludeSnapshots, thresholds);
			if (thresholds[0] > 0) {
				quota.setContainer(true); // set to true, so user see hard limit
											// not cluster size.
			}
		} else {
	...
	public IsilonSmartQuota constructIsilonSmartQuotaObjectWithThreshold(String path, String type, Long fsSize,
		...
		case 2:
			quota = new IsilonSmartQuota(path, type, thresholds[0], (thresholds[1] * size) / 100, 0L, 0L,
					bThresholdsIncludeOverhead, bIncludeSnapshots);
			break;
		case 3:
			quota = new IsilonSmartQuota(path, type, thresholds[0], (thresholds[1] * size) / 100,
					(thresholds[2] * size) / 100, 0L, bThresholdsIncludeOverhead, bIncludeSnapshots);
			break;
		case 4:
			quota = new IsilonSmartQuota(path, type, thresholds[0], (thresholds[1] * size) / 100,
					(thresholds[2] * size) / 100, (thresholds[3] * 60 * 60 * 24), bThresholdsIncludeOverhead,
					bIncludeSnapshots);
			break;
		default:
	 ...
	 * @throws IsilonException
	 */
	public IsilonList<IsilonSnapshot> listSnapshotsCreatedByPolicy(String resumeToken, String policy)
			throws IsilonException {
	...
	public String createSnapshot(String name, String path) throws IsilonException {
		return create(_baseUrl.resolve(URI_SNAPSHOTS), "snapshot", new IsilonSnapshot(name, path, null, null));
	}
	...
	public String createShare(String name, String path, String desc, String host) throws IsilonException {
		return create(_baseUrl.resolve(URI_SMB_SHARES), "share", new IsilonSMBShare(name, path, desc, host));
	}
	...
	public List<? extends IsilonPool> getDiskPools() throws IsilonException {
		IsilonList<IsilonDiskPool> pools = list(_baseUrl.resolve(URI_DISK_POOLS), "diskpools", IsilonDiskPool.class,
				null);
		return pools.getList();
	}
	...
	public List<IsilonNetworkPool> getNetworkPools(String resumeToken) throws IsilonException {
		IsilonList<IsilonNetworkPool> accessZoneIsilonList = list(_baseUrl.resolve(URI_NETWORK_POOLS), "pools",
				IsilonNetworkPool.class, resumeToken);
		return accessZoneIsilonList.getList();
	}
	...
	private IsilonList<IsilonEvent> getEvents(URI url, String firmwareVersion) throws IsilonException {
		...
		// Get list of ISILON events using eventlists if ISILON version is
		// OneFS8.0 or more else using events.
	...
	public IsilonList<IsilonEvent> queryEvents(long begin, long end, String firmwareVersion) throws IsilonException {
		...
		String query = (end != 0) ? String.format("?begin=%1$d&end=%2$d", begin, end)
				: String.format("?begin=%1$d", begin);

		// If ISILON version is OneFS8.0 then get events URI will be
	 ...
	 * @throws IsilonException
	 */
	public <T> HashMap<String, IsilonStats.StatValueCurrent<T>> getStatsCurrent(String key, Type valueType)
			throws IsilonException {
		ClientResponse clientResp = null;
		try {
			clientResp = _client
					.get(_baseUrl.resolve(URI_STATS.resolve(String.format("current?key=%1$s&devid=0", key))));
			if (clientResp.getStatus() != 200) {
	 ...
	 * @throws IsilonException
	 */
	public <T> HashMap<String, IsilonStats.StatValueHistory<T>> getStatsHistory(String key, long begin, Type valueType)
			throws IsilonException {
		ClientResponse clientResp = null;
		try {
			clientResp = _client.get(_baseUrl
					.resolve(URI_STATS.resolve(String.format("history?key=%1$s&devid=0&begin=%2$s", key, begin))));
			if (clientResp.getStatus() != 200) {
				...
				if (entryObj.has("values")) {
					...
					for (int i = 0; i < array.length(); i++) {
						...
						if (entry.length() == 2) {
							...
							T value = new Gson().fromJson(SecurityUtils.sanitizeJsonString(entry.getString(1)),
									valueType);
		...
		if (errorEntity != null) {
			if (errorEntity.has("errors")) {
				throw IsilonException.exceptions.processErrorResponseFromIsilonMsg(operationKey, objectKey, httpStatus,
						_baseUrl, errorEntity.getString("errors"));
			} else if (errorEntity.has("message")) {
				throw IsilonException.exceptions.processErrorResponseFromIsilonMsg(operationKey, objectKey, httpStatus,
						_baseUrl, errorEntity.getString("message"));
			}
		} else {
			throw IsilonException.exceptions.processErrorResponseFromIsilon(operationKey, objectKey, httpStatus,
					_baseUrl);
		}
	}
	...
	public boolean nfsv4Enabled(String firmwareVersion) throws IsilonException {
		...
		try {
			...
			// Check if ISILON ONEFS version is 8.0 and more to get NFSV4
			// details
	 ...
	 * @return boolean true if exists, false otherwise
	 */
	// TODO removed the unused parameter..this method is also not used by any
	// method..
	public boolean isSyncIQEnabled() throws IsilonException {
		ClientResponse resp = null;
	...
	public String getReplicationLicenseInfo() throws IsilonException, JSONException {
		...
		try {
			// Verify whether SyncIQ service is enabled on ISILON array or not
			resp = _client.get(_baseUrl.resolve(URI_REPLICATION_LICENSE_INFO));
			JSONObject jsonResp = resp.getEntity(JSONObject.class);
		...
		sLogger.info("Isilon SyncIQ license status is  {}", licenseStatus);
	...
	/**
	 * Get Replication Policy information from the Isilon array using oneFS v8
	 * above
	 * 
	 * @return IsilonSyncPolicy object
	 * @throws IsilonException
	...
	public IsilonList<IsilonSyncPolicyReport> getTargetReplicationPolicyReports(String policyName)
			throws IsilonException {
	 ...
	 * 
	 * @param licenseType
	 *            type of the license for which the activation status is
	 *            required
	...
	public String snapshotIQLicenseInfo() throws IsilonException, JSONException {
		...
		try {
			// Verify whether SnapshotIQ service is enabled on ISILON array or
			// not
