 ...
 * <p/>
 * 1. Multithreaded queue consumer callbacks
 * 2. Queue item is always processed with lock safety (meaning they are not removed from queue until successfully
 *    processed / lock released)
 */
public class DistributedQueueImpl<T> implements DistributedQueue<T> {
     ...
     * <p/>
     * 1. scans queued items and attempts to lock
     * 2. if locked, queues it to pending item list
     */
