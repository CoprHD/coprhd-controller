...
public class CreateNfsDatastore extends ExecutionTask<Datastore>{
	...
	private void handleCreationAttemptException(int retry, VMWareException e) {
		...
		info(e, "Attempt #%d to create the NFS Datastore failed. %d retries remaining. [%s]", retry, CREATE_RETRIES-retry+1, errorMessage);
    ...
    private RuntimeException handleKnownException(Exception e, String detailMessage, String... args ) {
        final String errorMessage = getMessage("CreateNfsDatastore.exception.boiler", mountPath, String.format(detailMessage, (Object[])args));
