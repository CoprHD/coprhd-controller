...
public class RestoreGroupActiveSyncStrategy implements ExecutorStrategy {
    private SmisCommandHelper helper;

    public RestoreGroupActiveSyncStrategy(SmisCommandHelper helper) {
        this.helper = helper;
    }

    @Override
    public void execute(Collection<CIMObjectPath> objectPaths, StorageSystem provider) throws WBEMException {
        CIMArgument[] args = helper.getActiveSRDFRestoreInputArguments(objectPaths.iterator().next());
