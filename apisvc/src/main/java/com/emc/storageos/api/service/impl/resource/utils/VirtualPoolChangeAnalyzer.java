...
public class VirtualPoolChangeAnalyzer extends DataObjectChangeAnalyzer {
    ...
    public static boolean isSupportedFileReplicationChange(VirtualPool currentVpool, VirtualPool newVpool, StringBuffer notSuppReasonBuff) {
        ...
        // Source virtual pool must not enabled with replication!!
        if (VirtualPool.vPoolSpecifiesFileReplication(currentVpool)) {
            notSuppReasonBuff
                    .append(String
                            .format("File replication is already enable in source virtual pool %s.",
                                    currentVpool.getLabel()));
            s_logger.info(notSuppReasonBuff.toString());
            return false;
        }

        // Target virtual pool must be enabled with replication!!
        if (!VirtualPool.vPoolSpecifiesFileReplication(newVpool)) {
            notSuppReasonBuff
                    .append(String
                            .format("File replication is not enable in target virtual pool %s.",
                                    newVpool.getLabel()));
            s_logger.info(notSuppReasonBuff.toString());
            return false;
        }
...
//                    if (VirtualPool.vPoolSpecifiesRPVPlex(currentTargetVpool)
//                            && VirtualPool.vPoolSpecifiesRPVPlex(newTargetVpool)) {
                        // Add this Target's currentVpool and the newVpool as a potential candidate for migrations
                    
                    
//                    if (candidatesForMigration.get(Volume.PersonalityTypes.TARGET) == null) {
//                        candidatesForMigration.put(Volume.PersonalityTypes.TARGET, new HashMap<VirtualPool, VirtualPool>());
//                    }
//                    candidatesForMigration.get(Volume.PersonalityTypes.TARGET).put(currentTargetVpool, newTargetVpool);
