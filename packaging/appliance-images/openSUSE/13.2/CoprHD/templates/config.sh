...
## BEGIN:/etc/init.d/boot.local - configure swap to boost VMs with low memory
#echo "if [ ! -f /var/tmp/swapfile ]; then" >> /etc/init.d/boot.local
#echo "dd if=/dev/zero of=/var/tmp/swapfile bs=1024 count=1048576" >> /etc/init.d/boot.local
#echo "mkswap -c -v1 /var/tmp/swapfile" >> /etc/init.d/boot.local
#echo "fi" >> /etc/init.d/boot.local
#echo "swapon /var/tmp/swapfile" >> /etc/init.d/boot.local
#chmod u+x /etc/init.d/boot.local
