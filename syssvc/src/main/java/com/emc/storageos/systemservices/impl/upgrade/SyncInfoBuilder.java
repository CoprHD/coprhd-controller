...
public class SyncInfoBuilder {
    ...
    public static List<SoftwareVersion> getInstallableRemoteVersions(final RepositoryInfo local,
        ...
        final String args = MessageFormat.format(
                "Number of local versions/maximum software versions allowed: [{0}/{1}] current version: {2} local versions: {3} ",
    ...
    private static boolean isRemovable(List<SoftwareVersion> versions,
        ...
        if (upgradeFromVersionList.isEmpty() || upgradeToVersionList.isEmpty()) {
            return true; // If no version can upgrade to the target version or no version can be upgrade from the target version, target
        ...
        return true; // All versions from the upgradeToVersionList have versions other than the target version to upgrade from, true should
                     // be returned
    }
