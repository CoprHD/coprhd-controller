...
public class DataObjectChangeAnalyzer {
    ...
    private static void analyzeStringSets(StringSet a, StringSet b, 
        ...
        if (a != null) {
            ...
            while (iter.hasNext()) {
                ...
                if (b != null && b.contains(val)) continue;
                String key = name + "." + val;
        ...
        if (b != null) {
            ...
            while (iter.hasNext()) {
                String val = iter.next();
                if (a != null && a.contains(val)) continue;
                String key = name + "." + val;
        ...
        if (a != null) {
            for (String key : a.keySet()) {
                if (b != null && b.containsKey(key) 
                        && a.get(key).equals(b.get(key))) continue;
        ...
        if (b != null) {
            for (String key : b.keySet()) {
                if (a != null && a.containsKey(key) 
                        && b.get(key).equals(a.get(key))) continue;
     ...
     * be processed.
     * <p>
     * This routine works by reflection and calls only the gettrs for fields
     * that are annotated with @Name("xxx"). It understands basic object types,
     * such as Boolean, String, Integer, and StringSet, StringMap, and
     * StringSetMap.
    ...
    public static Map<String, Change> analyzeChanges(
        ...
        if (excludedNames == null) excludedNames = new String[] {} ;
