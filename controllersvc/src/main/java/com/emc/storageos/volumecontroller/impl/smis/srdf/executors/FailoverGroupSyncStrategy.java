 ...
 */
public class FailoverGroupSyncStrategy implements ExecutorStrategy {
    private SmisCommandHelper helper;

    public FailoverGroupSyncStrategy(SmisCommandHelper helper) {
        this.helper = helper;
    }

    @Override
    public void execute(Collection<CIMObjectPath> objectPaths, StorageSystem provider) throws WBEMException {
        CIMArgument[] args = helper.getFailoverSyncPairInputArguments(objectPaths.iterator().next());
