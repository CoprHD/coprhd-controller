...
public class BrocadeNetworkSMIS extends BaseSANCIMObject {
     ...
     * for the alternate function {@link #getFCEndpointsByTopologyViewInFabric(WBEMClient, Map, Map, Map)}
     * which is faster but requires more memory. The user can select the function best suitable
     ...
     * {@link #getFCEndpointsByFabric(WBEMClient, Map, Map, Map)} which can be slow. The user 
     * can select the function best suitable using config item 
     * controller_ns_brocade_discovery_by_fabric_association
     * 
     * @param client 
     *        WBEMClient
     * @param routedConnections 
     ...
     *   ManagedElement = "root/brocade1:Brocade_ZoneAlias.InstanceID=\"NAME=Hala_REST_API_ZONE_12;ACTIVE=false;FABRIC=10000027F858F6C0;CLASSNAME=Brocade_ZoneAlias\"";
     *   SettingData = "root/brocade1:Brocade_ZoneMembershipSettingData.InstanceID=\"NAME=2010101010101010;ZMTYPE=5;ACTIVE=false;FABRIC=10000027F858F6C0;CLASSNAME=Brocade_ZoneMembershipSettingData\"";
     * };
     * 
     * @param client
     * @param fabricEps
    ...
    public boolean zonesetHasMore (WBEMClient client, CIMObjectPath zonesetPath, int removeCount) throws WBEMException {
        ...
        try {
            ...
            while (zoneItr.hasNext()) {
                if (zoneItr.next()!=null) count++;
                if (count > removeCount) return true;
            }
        } finally {
            if (zoneItr != null) {
                zoneItr.close();
    ...
    public List<ZoneMember> getZoneMembers (WBEMClient client,
        ...
        for (ZoneMember wwnMember : wwnMembers ) {
            ...
            if (!found) aliasMembers.add(wwnMember);
        }
        return aliasMembers;
    }
    ...
    private CIMInstance getFabricInstance(WBEMClient client, String fabricId, String fabricWwn)
            throws WBEMException {
        if (fabricWwn == null) fabricWwn = "";      // avoids null check below
        else fabricWwn = fabricWwn.replaceAll(":", "");
     ...
     * <p>
     * This function will retry if the alias creation fails because the session
     * is lost or timed out. The total number of tries is set to 2.
     * 
