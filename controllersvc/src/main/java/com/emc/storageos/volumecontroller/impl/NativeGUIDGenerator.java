...
public class NativeGUIDGenerator {
    ...
    public static String generateNativeGuid(NetworkSystem netDevice) {
        ...
        if (netDevice.getSystemType().equalsIgnoreCase(NetworkSystem.Type.brocade.name())
            ...
            return String.format("%s+%s+%s", _deviceTypeMap.get(netDevice.getSystemType()), netDevice.getSmisProviderIP(), netDevice.getSmisPortNumber());
        }
        
        return String.format("%s+%s+%s", _deviceTypeMap.get(netDevice.getSystemType()), netDevice.getIpAddress(), netDevice.getPortNumber());
    }
    ...
    public static String generateNativeGuid(DbClient dbClient, StoragePort port) {
        ...
        return String.format("%s+%s+"+PORT+"+%s", _deviceTypeMap.get(device.getSystemType()), device.getSerialNumber(),  port.getPortNetworkId());
    }
    ...
    public static String generateNativeGuid(DbClient dbClient, StorageHADomain adapter) {
        ...
        return String.format("%s+%s+"+ADAPTER+"+%s", _deviceTypeMap.get(device.getSystemType()), device.getSerialNumber(),  adapter.getAdapterName());
    }
     ...
     * @throws IOException
     */
    private static String generateSPNativeGuidFromIndication(Hashtable<String, String> cimIndication, String poolNativeId_IndicationAttribute) {
    ...
    {
        _logger.info("Device Type : {} Serial No : {} nativeId : {}", new Object[]{_deviceTypeMap.get(deviceType.getSystemType()), serialNumber, nativeId});
    ...
    public static String generateNativeGuidForPreExistingFileExportRule(StorageSystem storageSystem, String fileShareNativeId ) {
        return String.format("%s+%s+"+UN_MANAGED_FILE_EXPORT_RULE+"+%s", _deviceTypeMap.get(storageSystem.getSystemType()), storageSystem.getSerialNumber().toUpperCase(), fileShareNativeId);
    }
    
    public static String generateNativeGuidForPreExistingFileShare(StorageSystem storageSystem, String fileShareNativeId ) {
        return String.format("%s+%s+"+UN_MANAGED_FILE_SHARE+"+%s", _deviceTypeMap.get(storageSystem.getSystemType()), storageSystem.getSerialNumber().toUpperCase(), fileShareNativeId);
    }
