...
public abstract class AnnotatedAssetOptionsProvider extends AbstractAssetOptionsProvider implements AssetOptionsProvider {
	 ...
	 * <code>public List<AssetOption> (AssetOptionsContext context, ...)</code> 
	 */
	private boolean isAssetMethod( Method javaMethod ) {
		...
		if ( isAnnotated && !correctSignature ) {
			log.error(String.format("Method %s::%s has the @Asset annotation but does not conform to the correct signature: public List<%s> (AssetOptionsContext context, ...)",
	...
	public List<AssetOption> invokeAssetMethod(AssetOptionsMethodInfo assetMethod, AssetOptionsContext context, Map<String, String> availableAssets)
    ...
    public AssetOptionsMethodInfo findAssetMethodInfo(String assetTypeName, Set<String> availableAssets)  {
        ...
        if (foundMatch == null) {
        	final String errorMessage = String.format("Query for Asset '%s' requires additional asset dependencies. Supplied: %s", assetTypeName, availableAssets.toString());
