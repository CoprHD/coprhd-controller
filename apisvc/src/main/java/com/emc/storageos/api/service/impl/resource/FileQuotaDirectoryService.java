...
public class FileQuotaDirectoryService extends TaskResourceService {
	...
	protected SearchedResRepList getNamedSearchResults(String name, URI projectId) {
		...
		if (projectId == null) {
			_dbClient.queryByConstraint(PrefixConstraint.Factory.getLabelPrefixConstraint(getResourceClass(), name),
					resRepList);
		} else {
			_dbClient.queryByConstraint(
					ContainmentPrefixConstraint.Factory.getFileshareUnderProjectConstraint(projectId, name),
					resRepList);
		}
		return resRepList;
	}
	...
	public TaskResourceRep updateQuotaDirectory(@PathParam("id") URI id, QuotaDirectoryUpdateParam param)
		...
		if (param.getSize() != null) {
			Long quotaSize = SizeUtil.translateSize(param.getSize());// converts
																		// the
																		// input
																		// string
																		// in
	...
	/**
	 * Deactivate Quota directory of file system, this will move the Quota
	 * directory to a "marked-for-delete" state
	 * <p>
	 * NOTE: This is an asynchronous operation.
	 * 
	 * @param id
	...
	public TaskResourceRep deactivateQuotaDirectory(@PathParam("id") URI id, QuotaDirectoryDeleteParam param)
		...
		// <TODO> Implement Force delete option when shares and exports for
		// Quota Directory are supported
		...
		try {
			...
			// If delete operation is successful, then remove obj from ViPR db
			// by setting inactive=true
