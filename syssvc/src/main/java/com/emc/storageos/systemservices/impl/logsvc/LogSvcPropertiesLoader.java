...
public class LogSvcPropertiesLoader {
    ...
    // service.
    private @Value("#{logsvcProperties['logsvc.logFilePaths']}") String _logFilePaths;
    ...
    private @Value("#{logsvcProperties['logsvc.nodeLogCollectionTimeout']}") int _nodeLogCollectorTimeout;

    // The timeout in seconds when waiting for a file log collector to complete
    // log collection from the log file.
    private @Value("#{logsvcProperties['logsvc.fileLogCollectionTimeout']}") int _fileLogCollectorTimeout;

    // The timeout in SECONDS when waiting to connect to a node.
    private @Value("#{logsvcProperties['logsvc.nodeLogConnectionTimeout']}") int _nodeLogConnectionTimeout;

    // The timeout in MILLISECONDS when waiting to collect logs from all nodes.
    private @Value("#{logsvcProperties['logsvc.logCollectionTimeout']}") long logCollectionTimeout;

    // The expiration time in MINUTES for dynamic log level changes via REST API.
    private @Value("#{logsvcProperties['logsvc.logLevelExpiration']}") int _logLevelExpiration;

    // This number is multiplied to file size to get zipped size
    private @Value("#{logsvcProperties['attachment.zipfactor']}") int zipFactor;

    // The maximum attachments size for ConnectEMC events in MB.
    private @Value("#{logsvcProperties['attachment.maxSize.MB']}") int attachmentMaxSizeMB;
