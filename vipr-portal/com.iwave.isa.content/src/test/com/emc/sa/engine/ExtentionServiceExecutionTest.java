...
public class ExtentionServiceExecutionTest {
	...
	public static void main(String[] args) throws Exception {
		ExtentionServiceExecutionTest exs = new ExtentionServiceExecutionTest();
		exs.setUp();
//		exs.testSimpleService();

//		String[] locations = { "classpath:service-runner.xml" };
//        ApplicationContext ac = new ClassPathXmlApplicationContext( locations );
        
		DefaultExecutionServiceFactory def = new DefaultExecutionServiceFactory();
//		def.setApplicationContext(ac);
		ExecutionService ser1= new GenericPluginService();
		
		List<ExecutionService> values = Arrays.asList(ser1);
		def.setServices(values);

		//Order order = new Order();
		Order order=exs.createOrder("CustomSample@Extension", "value=Test", "size=10");
		CatalogService catalogService = new CatalogService();
		catalogService.setBaseService("CustomSample@Extension");

		ExecutionService service = def.createService(order, catalogService);
		
		exs.createEngine(service);
		exs.executeOrder(service, order);
