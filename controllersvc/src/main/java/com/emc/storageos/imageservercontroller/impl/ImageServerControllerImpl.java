		...
		try {
			...
			if (!imageServerVerified) {
				...
				if (!imageServerVerified) {// still not verified
					throw ImageServerControllerException.exceptions.imageServerNotSetup("Can't perform image import: " + imageServerErrorMsg);
				}
			}
	...
	public void waitForFinishMethod(URI jobId, String stepId) {
		...
		try {
			...
			if (status == OsInstallStatus.SUCCESS) {
				...
				dbClient.persistObject(job);
				WorkflowStepCompleter.stepFailed(stepId, ImageServerControllerException.exceptions.osInstallationFailed("failure in the post-install"));
			} else { // timed out
