...
public class PortPoolCompatibilityStatusMigrationTest extends
	...
	protected void verifyResults() throws Exception {
		...
		while (portObjs.hasNext()) {
			...
			Assert.assertEquals("CompatibilityStatus should equal parent storage system's compatibility status", storageSystem.getCompatibilityStatus(),
				port.getCompatibilityStatus());
		}
		Assert.assertTrue("We should still have " + 4*INSTANCES_TO_CREATE + " " + StoragePort.class.getSimpleName() + " after migration, not " + portCount, portCount == 4*INSTANCES_TO_CREATE); 
		//verify pools data
		...
		Assert.assertTrue("We should still have " + 4*INSTANCES_TO_CREATE + " " + StoragePort.class.getSimpleName() + " after migration, not " + poolCount, poolCount == 4*INSTANCES_TO_CREATE); 
