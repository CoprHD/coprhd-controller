...
public class HostClusters extends Controller {
	...
	public static void listJson() {
		List<ClusterRestRep> clusters = ClusterUtils.getClusters(Models
				.currentAdminTenant());
		List<HostClusterDataTable.HostClusterInfo> hostClusterInfos = Lists
				.newArrayList();
		for (ClusterRestRep cluster : clusters) {
			hostClusterInfos.add(new HostClusterDataTable.HostClusterInfo(
					cluster));
		}
	...
	public static void itemDetails(String id) {
		...
		ProjectRestRep project = ProjectUtils.getProject(ResourceUtils
				.id(cluster.getProject()));
		List<HostRestRep> hosts = ClusterUtils.getHosts(uri(id));
		VcenterDataCenterRestRep dataCenter = VCenterUtils
				.getDataCenter(ResourceUtils.id(cluster.getVcenterDataCenter()));
	...
	public static void availableHostJson(String id) {
		...
		DefaultResourceFilter<HostRestRep> defaultHostResourceFilter = new DefaultResourceFilter<HostRestRep>() {
			...
			public boolean accept(HostRestRep hostRestRep) {
				return hostRestRep.getCluster() == null
						|| !hostRestRep.getCluster().getId().equals(clusterId);
			}
		};
		...
		if (!existingHosts.isEmpty()) {
			FilterChain<HostRestRep> hostTypeFilter = new FilterChain<HostRestRep>(
					new HostTypeFilter(existingHosts.get(0).getType()));

			hosts = getViprClient().hosts().getByTenant(
					cluster.getTenant().getId(),
					hostTypeFilter.and(defaultHostResourceFilter));
		} else {
			hosts = getViprClient().hosts().getByTenant(
					cluster.getTenant().getId(), defaultHostResourceFilter);
		}
	...
	public static class HostClusterForm {
		...
		public HostClusterForm() {
			...
			this.autoUnexportEnabled = Boolean.TRUE;
		}
		...
		protected void clearOtherErrors(String formName, String... fieldsToKeep) {
			...
			for (String name : fieldsToKeep) {
				play.data.validation.Error error = Validation.error(String
						.format("%s.%s", formName, name));
				if (error != null) {
					errors.add(error);
				}
			}
		...
		protected String createCluster() {
			...
			return ClusterUtils.createCluster(tenantId, clusterCreateParam)
					.toString();
		}
		...
		protected String updateCluster() {
			...
			return ClusterUtils.updateHost(uri(this.id), hostUpdateParam)
					.toString();
		}
		...
		protected String updateCluster(HostClusterForm hostCluster) {
			...
			hostUpdateParam
					.setAutoUnexportEnabled(hostCluster.autoUnexportEnabled);
