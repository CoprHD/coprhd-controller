...
public class HP3PARStorageDriver extends AbstractStorageDriver implements BlockStorageDriver {
			...
			for (int volIndex = 0; volIndex < objStorageVolumes.getTotal(); volIndex++) {
				...
				if( objVolMember.getCopyOf() != null){
					...
					if(objVolMember.getCopyType()==copyType.VIRTUAL_COPY.getValue()){
						...
						_log.info("objVolMember.getBaseId() is {}",objVolMember.getBaseId());
						VolumeAncestorInfo objAncestor = idToNameMap.get(objVolMember.getBaseId());
				...
				_log.info("AFTER THE IF CONDITION");
		...
		for (StorageVolume volume : volumes) {
			try {
				...
				String msg = String.format(
						...
						volume.getDisplayName(), volume.getStoragePoolId(), volume.getStorageSystemId(),
						e.getMessage());
				_log.error(msg);
				task.setMessage(msg);
	...
	public List<VolumeSnapshot> getVolumeSnapshots(StorageVolume volume) {
		...
		try {
			...
			for (int i = 0; i < listOfChildVols.size() ; i++) {
				...
				if(objSnapshot.getCopyType() != copyType.VIRTUAL_COPY.getValue()){
					//if the copy type is physical, then its not a snapshot, hence
					//we disregard it.
					continue;
				}
	...
	/**
	 * restore clone is equivalent to promote physical copy Here we will be
	 * using intermediate snapshot for restore which got generated during clone
	 * creation NOTE: intermediate snapshot cannot be exported hence offline
	 * restore will be used
	 */
	@Override
	...
	public Map<String, HostExportInfo> getVolumeExportInfoForHosts(StorageVolume volume) {
		...
		return getStorageObjectExportInfoForHosts(volume.getStorageSystemId(), volume.getWwn() , volume.getNativeId(), volume);		
	}

	public Map<String, HostExportInfo> getStorageObjectExportInfoForHosts(String storageSysmId, String objectWWN, String volumeName, StorageBlockObject blockObject) {
		_log.info("3PARDriver: getVolumeExportInfoForHosts Running ");
		...
		try {			
			...
			HP3PARApi hp3parApi = getHP3PARDeviceFromNativeId(storageSysmId);
			VirtualLunsList vlunsOfVolume = hp3parApi.getVLunsOfVolume(objectWWN);
			...
			for (int index = 0; index < vlunsOfVolume.getTotal(); index++) {
				...
				// node:portpos:cardport combination of the VLUN
				List<StoragePort> storPortsOfStorage = storagePortMap.get(storageSysmId);
			...
			String msg = String.format(
					...
					volumeName, storageSysmId, e.getMessage());
			_log.error(msg);
			e.printStackTrace();
		}
	...
	public Map<String, HostExportInfo> getSnapshotExportInfoForHosts(VolumeSnapshot snapshot) {
		...
		return getStorageObjectExportInfoForHosts(snapshot.getStorageSystemId(), snapshot.getWwn() , snapshot.getNativeId(), snapshot);		
	}
	 ...
	 * 
	 * 1 Export volume to existing host * 2 Export volume to non-existing host 3
	 * Add initiator to existing host 4 Remove initiator from host 5 Unexport
	 * volume -- (SDK break?)
	 * 
	 * A 1-5 can be done with single/multiple volumes,initiators as applicable B
	 * Should not depend on host name C Adding an initiator to host will not do
	 * anything further. All volumes have to be exported to new initiator
	 * explicitly -------------------------- SHARED EXPORT: Will not include
	 * port number, exported to all ports cluster can see
	 * 
	 * 1 Export volume to existing cluster (SDK gives n host calls=>export to
	 * individual host) 2 Export volume to non-existing cluster 3 Add initiator
	 * to existing host in cluster 4 Remove initiator from host in cluster 5
	 * Unexport volume from cluster 6 Export volume to specific host in a
	 * cluster 7 Unexport volume from a specific host in a cluster 8 Add a host
	 * to cluster 9 Remove a host from a cluster 10 Add a host having private
	 * export 11 Remove a host having private export 12 Move a host from one
	 * cluster to another
	 * 
	 ...
	 * the new host(earlier export must have been host-set)
	 */
	...
	public DriverTask exportVolumesToInitiators(List<Initiator> initiators, List<StorageVolume> volumes,
		...
		try {
			...
			if (initiators.get(0).getInitiatorType().equals(Type.Host) == true) {
				...
				if (host == null) {
					...
					// TODO: to be handled for each volume as it may be from
					// different array
					...
					Integer persona = 0;
					// Supporting from lower versions; Windows1, HPUX7, Linux1,
					// Esx11, AIX8, AIXVIO8, SUNVCS1, No_OS6, Other6
					...
					// volume export
						 ...
						 * else if
						 * (initiators.get(0).getInitiatorType().equals(Type.
						 * Cluster) == true) { // Shared-Cluster export
						 * 
						 * 
						 * } else { _log.error(
						 * "3PARDriver:exportVolumesToInitiators error: Host type not supported"
			...
			/*
			 * try { // 1 This could be single host export // 2 There could be
			 * single host export for the cluster (private export) // 3 or this
			 * could be one of the host export for the entire cluster
			 * 
			 * // From initiator port wwn/iqn get the hostname registered with
			 * 3PAR // host name registered with 3PAR is same across all arrays
			 * host = get3parHostname(initiators,
			 * volumes.get(0).getStorageSystemId()); if (host == null) { // if
			 * clusterName == null, create single host // get list of initiators
			 * and create single host
			 * 
			 * // clusterName present; 1 New cluster 2 Existing cluster // 2a
			 * new host addition // 1a create host, create cluster, add the host
			 * to cluster
			 * 
			 * // ++ create a host on 3par with this information
			 * 
			 * _log.error(
			 * "3PARDriver:exportVolumesToInitiators error in processing host name"
			 * ); throw new HP3PARException(
			 * "3PARDriver:exportVolumesToInitiators error in processing host name"
			 * ); }
			 */
		...
		for (StorageVolume vol : volumes) {
			...
			try {
				// volume could belong to different storage system; get specific
				// api client;
				...
				for (StoragePort port : recommendedPorts) {
					...
					if (vlunRes != null && vlunRes.getStatus() == true) {
						...
						// update hlu obtained as lun from 3apr & add the
						// selected port if required
						...
						task.setStatus(DriverTask.TaskStatus.PARTIALLY_FAILED);
						_log.warn("Could not export " + message);
					}
	...
	/*
	 * Single initiator might have multiple volumes and single volume could be
	 * exported to multiple initiators All volumes will be tried for unexport
	 * from all initiators
	 */
	@Override
	public DriverTask unexportVolumesFromInitiators(List<Initiator> initiators, List<StorageVolume> volumes) {
		...
		try {
			...
			String msg = String.format("3PARDriver:unexportVolumesFromInitiators error : %s", e.getMessage());
			_log.error(msg);
			task.setMessage(msg);
			task.setStatus(DriverTask.TaskStatus.FAILED);
			e.printStackTrace();
			...
			try {
				...
				for (int index = 0; index < vlunRes.getTotal(); index++) {
					...
					for (Initiator init : initiators) {
						...
						/*
						 * Multiple host initiators can map to same or different
						 * array target ports hence port can not be determined
						 * Currently unexport will happen from all array ports
						 * as we need to get this info from SDK
						 */
