...
public class VPlexProtectionRecommendation extends VPlexRecommendation {
    ...
    public String toString(DbClient dbClient) {
        ...
        String sourceRPSiteName = (ps.getRpSiteNames() != null) ? ps.getRpSiteNames().get(getSourceInternalSiteName())
        ...
        if (this.varrayVPlexProtection != null) {
            for (Map.Entry<URI, VPlexProtection> varrayProtectionEntry : this.varrayVPlexProtection.entrySet()) {
                ...
                StoragePool targetJournalStoragePool = (StoragePool) dbClient.queryObject(StoragePool.class,
                        vplexProtection.getTargetJournalStoragePool());
                ...
                if (vplexProtection.getTargetVPlexHaRecommendations() != null
                    ...
                    String targetHaVpool = (vplexTargetHaRec.getVirtualPool() != null) ? vplexTargetHaRec.getVirtualPool().getLabel()
                            : "N/A";
        ...
        if (this.getVirtualArrayProtectionMap() != null) {
            for (URI varrayId : this.getVirtualArrayProtectionMap().keySet()) {
                ...
                StoragePool targetPool = (StoragePool) dbClient.queryObject(StoragePool.class,
                        this.getVirtualArrayProtectionMap().get(varrayId).getTargetStoragePool());
                ...
                VirtualArray targetJournalVarray = dbClient.queryObject(VirtualArray.class,
                        this.getVirtualArrayProtectionMap().get(varrayId).getTargetJournalVarray());
                ...
                StorageSystem targetSystem = (StorageSystem) dbClient.queryObject(StorageSystem.class, this.getVirtualArrayProtectionMap()
                        .get(varrayId).getTargetDevice());
        ...
        if (vplexDistributed) {
            ...
            if (getSourceVPlexHaRecommendations().get(0) instanceof VPlexProtectionRecommendation) {
                ...
                if (protectionRec.getProtectionDevice() != null && protectionRec.getSourceInternalSiteName() != null) {
                    ...
                    String haSourceRPSiteName = (haPs.getRpSiteNames() != null) ? haPs.getRpSiteNames().get(
                            protectionRec.getSourceInternalSiteName()) : protectionRec.getSourceInternalSiteName();
                ...
                if (protectionRec.varrayVPlexProtection != null) {
                    for (Map.Entry<URI, VPlexProtection> varrayProtectionEntry : protectionRec.varrayVPlexProtection.entrySet()) {
                        ...
                        StoragePool targetJournalStoragePool = dbClient.queryObject(StoragePool.class,
                                vplexProtection.getTargetJournalStoragePool());
                        VirtualArray targetJournalVarray = dbClient.queryObject(VirtualArray.class,
                                vplexProtection.getTargetJournalVarray());
                        VirtualPool targetJournalVpool = dbClient.queryObject(VirtualPool.class, vplexProtection.getTargetJournalVpool());
                        String targetInternalSiteName = vplexProtection.getTargetInternalSiteName();
                        StorageSystem vplexTargetSystem = dbClient.queryObject(StorageSystem.class, vplexProtection.getTargetVplexDevice());
                        String targetRPSiteName = (ps.getRpSiteNames() != null) ? ps.getRpSiteNames().get(targetInternalSiteName)
                        ...
                        if (vplexProtection.getTargetVPlexHaRecommendations() != null
                                && !vplexProtection.getTargetVPlexHaRecommendations().isEmpty()) {
                            VPlexRecommendation vplexTargetHaRec = (VPlexRecommendation) vplexProtection.getTargetVPlexHaRecommendations()
                                    .get(0);
                            String targetHaVarrayStr = "N/A";
                            if (vplexTargetHaRec.getVirtualArray() != null) {
                                VirtualArray targetHaVarray = dbClient.queryObject(VirtualArray.class, vplexTargetHaRec.getVirtualArray());
                            ...
                            StorageSystem targetHaVplex = dbClient.queryObject(StorageSystem.class,
                                    vplexTargetHaRec.getVPlexStorageSystem());
                            StorageSystem targetHaStorageSystem = dbClient.queryObject(StorageSystem.class,
                                    vplexTargetHaRec.getSourceDevice());
                            StoragePool targetHaStoragePool = dbClient.queryObject(StoragePool.class, vplexTargetHaRec.getSourcePool());
                            String targetHaVpool = (vplexTargetHaRec.getVirtualPool() != null) ? vplexTargetHaRec.getVirtualPool()
                                    .getLabel() : "N/A";
                            buff.append("\tHA Cluster Virtual Array : " + targetHaVarrayStr + "\n");
                            buff.append("\tHA Cluster Virtual Pool : " + targetHaVpool + "\n");
                            buff.append("\tHA Cluster VPlex Storage System : " + targetHaVplex.getLabel() + "\n");
                ...
                if (protectionRec.getVirtualArrayProtectionMap() != null) {
                    for (URI varrayId : protectionRec.getVirtualArrayProtectionMap().keySet()) {
                        VirtualArray varray = (VirtualArray) dbClient.queryObject(VirtualArray.class, varrayId);
                        StoragePool targetPool = (StoragePool) dbClient.queryObject(StoragePool.class, protectionRec
                                .getVirtualArrayProtectionMap().get(varrayId).getTargetStoragePool());
                        VirtualArray targetJournalVarray = dbClient.queryObject(VirtualArray.class, protectionRec
                                .getVirtualArrayProtectionMap().get(varrayId).getTargetJournalVarray());
                        VirtualPool targetJournalVpool = dbClient.queryObject(VirtualPool.class, protectionRec
                                .getVirtualArrayProtectionMap().get(varrayId).getTargetJournalVpool());
                        StoragePool targetJournalPool = dbClient.queryObject(StoragePool.class, protectionRec
                                .getVirtualArrayProtectionMap().get(varrayId).getTargetJournalStoragePool());
                        StorageSystem targetSystem = (StorageSystem) dbClient.queryObject(StorageSystem.class, protectionRec
                                .getVirtualArrayProtectionMap().get(varrayId).getTargetDevice());
                        String targetInternalSiteName = protectionRec.getVirtualArrayProtectionMap().get(varrayId)
