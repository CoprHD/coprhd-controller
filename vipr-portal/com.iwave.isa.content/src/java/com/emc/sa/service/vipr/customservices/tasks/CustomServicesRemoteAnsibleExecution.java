...
public class CustomServicesRemoteAnsibleExecution extends ViPRExecutionTask<CustomServicesTaskResult> {
    ...
    public CustomServicesTaskResult executeTask() throws Exception {
        ...
        ExecutionUtils.currentContext().logInfo("customServicesScriptExecution.statusInfo", step.getId()+ "\t Step Name:" + step.getFriendlyName());
        ...
        try {
            ...
            if (null == primitive) {
                ...
                ExecutionUtils.currentContext().logError("customServicesOperationExecution.logStatus", step.getId()+ "\t Step Name:" + step.getFriendlyName(), "\"Error retrieving the Remote Ansible primitive from DB.");
        ...
        if (result == null) {
            ExecutionUtils.currentContext().logError("customServicesOperationExecution.logStatus", step.getId()+ "\t Step Name:" + step.getFriendlyName(),"Remote Ansible execution Failed");
        ...
        if (!StringUtils.isEmpty(parsedOut)) {
            ExecutionUtils.currentContext().logInfo("customServicesScriptExecution.doneInfo", step.getId()+ "\t Step Name:" + step.getFriendlyName() + "Execution Output:" + parsedOut);
