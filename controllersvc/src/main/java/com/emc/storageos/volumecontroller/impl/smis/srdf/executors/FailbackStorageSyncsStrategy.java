 ...
 */
public class FailbackStorageSyncsStrategy implements ExecutorStrategy {

    private SmisCommandHelper helper;

    public FailbackStorageSyncsStrategy(SmisCommandHelper helper) {
        this.helper = helper;
    }

    @Override
    public void execute(Collection<CIMObjectPath> objectPaths, StorageSystem provider) throws WBEMException {
        CIMArgument[] args = helper.getASyncPairFailBackInputArguments(objectPaths);
