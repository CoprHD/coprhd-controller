...
import static com.emc.ctd.vipr.api.ViPRClientFactory.getViprClient;
...
public class ViPRClientUtils {
	...
	public static void deactiveVolume(String volumeId) {
		getViprClient().blockVolumes().deactivate(uri(volumeId)).waitFor();
	}


	public static void deactiveBlockExport(String blockExportId) {
		getViprClient().blockExports().deactivate(uri(blockExportId)).waitFor();
	...
	public static  Task<ExportGroupRestRep> createHostOrClusterExport(URI varrayId, URI projectId, List<URI> volumeIds, Integer hlu, boolean isCluster, URI hostOrClusterId) {
		 ...
		 List<ExportGroupRestRep> exportList = null;
        if (!isCluster){
        	exportList = getViprClient().blockExports().findByHost(hostOrClusterId, projectId, varrayId);
        } else {
        	exportList = getViprClient().blockExports().findByCluster(hostOrClusterId, projectId, varrayId);
        }

        if(exportList.isEmpty()){
    		
            ExportCreateParam exportCreate = new ExportCreateParam();

            exportCreate.setVarray(varrayId);
            exportCreate.setProject(projectId);
            Integer currentHlu = hlu;
            ...
            for (URI volumeId : volumeIds) {
                ...
                exportCreate.getVolumes().add(volume);
            }

            if (isCluster) {
            	exportCreate.setName(getViprClient().clusters().get(hostOrClusterId).getName());
                exportCreate.addCluster(hostOrClusterId);
                exportCreate.setType("Cluster");
            }
            else {
            	exportCreate.setName(getViprClient().hosts().get(hostOrClusterId).getName());
            	exportCreate.addHost(hostOrClusterId);
                exportCreate.setType("Host");
            }
            return getViprClient().blockExports().create(exportCreate);
            ...
            return getViprClient().blockExports().update(exportList.get(0).getId(), export);
        }
