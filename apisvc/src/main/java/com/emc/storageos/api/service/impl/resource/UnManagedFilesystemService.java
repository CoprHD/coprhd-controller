...
public class UnManagedFilesystemService extends TaggedResource {
   	    ...
   	    ArgValidator.checkUri(param.getProject());
            ...
            for (URI unManagedFileSystemUri : param.getUnManagedFileSystems()) {
                ...
                if (pool.getIsResourceLimitSet()) {
                    if (pool.getMaxResources() <= 
                        StoragePoolService.getNumResources(pool, _dbClient) ) {
                ...
                if (system.getIsResourceLimitSet()) {
                    if (system.getMaxResources() <= 
                        StorageSystemService.getNumResources(system, _dbClient) ) {
    ...
    private List<UnManagedFileExportRule> queryDBFSExports(UnManagedFileSystem fs)
    {
    	...
    	try{
	    	ContainmentConstraint  containmentConstraint = ContainmentConstraint.Factory.getUnManagedFileExportRulesConstraint(fs.getId());
	   	  	List<UnManagedFileExportRule> fileExportRules = CustomQueryUtility.queryActiveResourcesByConstraint(_dbClient, UnManagedFileExportRule.class,
    ...
    /**
     * Check to see if StoragePort has a StorageHADomain associated (for example in vnxfile case), if does check all the ports that belong to the 
     * StorageDomain and verify to see if any of the ports belong to Neighborhood. If there is a match, associate that port to Filesystem's StoragePort feild. 
     * If StorageHADomain doesn't exists for port (in case of netapp and isilon). Get all the ports for storagesystem and select a random storageport to be 
