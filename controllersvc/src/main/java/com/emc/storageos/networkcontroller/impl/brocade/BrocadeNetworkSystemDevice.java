...
public class BrocadeNetworkSystemDevice extends NetworkSystemDeviceImpl
	 ...
	 * rules for creating a zone are:<ul>
	 * <li>If an active zone with the same name exists, ensure that all the desired members
	 * are in the zone. If this is true, consider the zone created. If not, error because 
	 * the application is not going to modify an existing zone.</li>
	 * <li>If an inactive zone with the same name exists, delete the inactive zone and then
	 * create the new one.</li>
	 * </ul> 
	...
	private boolean createZone(WBEMClient client, CIMInstance zoneServiceIns,
		...
		if (zonePath != null) {
            boolean success = false; String name = null;
			for (ZoneMember member : zone.getMembers()) {
	 ...
	 * <p>
	 * Removing zones typical flow is: <ol>
	 ...
	 * </ol>
	 * This flow is different when we're removing the last zones in a zoneset.
	 * If the zoneset becomes empty, it needs to be removed too or commit would 
	 * fail. In order to remove the zoneset, it has to first be deactivated. The 
	 * flow for removing the last zones in a zoneset is<ol>
     * <li>find the zones that can be deleted</li>
    ...
    public Map<String, String> updateZonesStrategy(WBEMClient client, List<ZoneUpdate> zones,
            String fabricId, String fabricWwn, boolean activateZones) throws NetworkDeviceControllerException { // to do - Make sure fabric id and fabric wwn are not null or only request needed params
        // a zone-name-to-result map to hold the results for each zone
     ...
     * <p>
     * If the zone already has the WWN of an alias and now the user is trying
     * to add the alias into the zone, the alias will not be added. The WWN
     * has to be removed first. 
     * <p>
     * Replacing a WWN with its alias should be possible in a single call
     * by specifying the WWN in the remove list and alias in the add list.
     * <p>
     ...
     * from the SMIS provider. <ol> 
     * <li>The default way of getting TopologyView instances is by fabric. This can be slow for
     * some configurations</li> 
     * <li> The alternative way is to get all TopologyView instances and group them into theit
     * fabrics using DependentSystem property. This does not work well for FDMI hosts as multiple
     * associations need to be traversed. This mode should only be used by customer not deploying
     * FDMI</li>
     * </ol> 
     * @return
     */
