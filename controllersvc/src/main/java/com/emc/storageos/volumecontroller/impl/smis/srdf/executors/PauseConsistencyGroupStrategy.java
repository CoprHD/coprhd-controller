 ...
 */
public class PauseConsistencyGroupStrategy implements ExecutorStrategy {

    private SmisCommandHelper helper;

    public PauseConsistencyGroupStrategy(SmisCommandHelper helper) {
        this.helper = helper;
    }

    @Override
    public void execute(Collection<CIMObjectPath> objectPaths, StorageSystem provider) throws WBEMException {
        CIMObjectPath first = objectPaths.iterator().next();
        CIMArgument[] args = helper.getSRDFPauseGroupInputArguments(first, null, true); // true == Split
