...
class ExportPathPolicy(object):

    '''
    The class definition for operations on Export Path Policies.
    '''

    URI_PATHPOLICY="/block/export-path-policies"
    ...
    def path_policy_list_uris(self, vdcname=None, tenant=None):
        '''
        This function will give us the list of export-path-policy uris
        separated by comma.
        '''
        vdcuri = None
        vdcrestapi = None
        ...
        if(tenant != None):
            ...
            if(vdcname != None):
                ...
                vdcrestapi = self.URI_PATHPOLICY
        (s, h) = common.service_json_request(
            self.__ipAddr, self.__port,
            "GET", vdcrestapi, None)
        o = common.json_decode(s)
        return o['export_path_policy']

def path_policy_list(args):
         '''
         this function is wrapper to the export-path-policies uris
         to give the list of vpool uris.
         '''
         vdcname=None
         tenant=None
         obj = ExportPathPolicy(args.ip, args.port)
         uris = obj.path_policy_list_uris(vdcname, tenant)
         #print 'uris: '
         #print uris
         res = common.list_by_hrefs(args.ip, args.port, uris)
         #print res
         common.TableGenerator(res, ['name', 'min_paths', 'max_paths', 'paths_per_initiator', 'max_initiators_per_port', 'description']).printTable();
...
def list_parser(subcommand_parsers, common_parser):
        ...
        list_parser = subcommand_parsers.add_parser(
            'list',
            description='ViPR Export Path Policy List CLI usage',
            parents=[common_parser],
            conflict_handler='resolve',
            help='List Export Path Policies')
        list_parser.add_argument('-v', '-verbose',
                              dest='verbose',
                              help='List VPool with details',
                              action='store_true')
        list_parser.set_defaults(func=path_policy_list)
...
def pathpolicy_parser(parent_subparser, common_parser):
        # main vpool parser
        parser = parent_subparser.add_parser('pathpolicy',
                                          description='ViPR Export Path Policy cli usage',
                                          parents=[common_parser],
                                          conflict_handler='resolve',
                                          help='Operations on Export Path Policy')
