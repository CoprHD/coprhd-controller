...
/**
 * Extend InternnodeAuthenticator to support blacklist to refuse gossip connection
 * from nodes in remote sote.
 * 
 * The blacklist is stored in ZK under /config/geodbconfig. The blacklist is reloaded
 * each time during service startup.
 */
public class BlacklistAuthenticator implements IInternodeAuthenticator, BlacklistAuthenticatorMBean {
    private static final Logger log = LoggerFactory.getLogger(BlacklistAuthenticator.class);
