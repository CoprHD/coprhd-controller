...
public class NetworkService extends TaggedResource {
     ...
     * <p>
     * If network systems are discovered, fiber channel networks that are discovered
     * are not initially associated with virtual array. The discovered networks must
     * be updated to associate then with virtual arrays.
     * @brief List Networks
    ...
    public NetworkList getAllNetworks() {
        ...
        for (Network network : transportZones) {
            if (network == null || network.getInactive() == true) continue;
     ...
     * <p>
     * For fiber channel, some Networks may be automatically created by discovering Network Systems.
     * These Networks will have endpoints that were discovered by a Network System, 
     * including endpoints that represent host initiator port WWNs as well as end points that
     * represent storage array port WWNs.
     * <p>
     * Discovered endpoints may not be deleted by the user. They will be updated periodically as the
     * Network System refreshes its information on the topology of the VSANs or Fabrics.
     * <p>
     * The user may still manually add endpoints to a discovered Network. The user is able to delete
     * endpoints that were manually added. If a manually entered endpoint is subsequently discovered by a
     ...
     * <li>The networks, the one the endpoints are added into, and the old
     * one when an endpoint is moving from one network to another</li>
     * <li>The storage pools when this operation results in the pools being
     * implicitly associated with the new network's varray and
     ...
     * <li>If the ports exists in another network, its endpoint is
     * removed from the old network and the pool-to-varray
     * associations are also updated</li>
     * </ul>
    ...
    * <p>
    * When the network has endpoints and the endpoints are matched to storage ports,
    * the storage ports become assigned to the network. When the network is also 
    * added to virtual arrays, the storage ports' array pools are update to show they
    * are connected to the networks' varrays.
    *   
   ...
   public NetworkRestRep createNetwork(NetworkCreate param) {
     ...
     * storage ports, the storage ports associations to the network are updated accordingly.
     *  If the endpoints added exist is another network, they are first removed from their 
     * current network. Discovered endpoints cannot be removed from their current networks
     * or added to another one.
     * <p>
     * When the storage ports networks are changed, their corresponding storage pools are 
     * also update to reflect any change in varray connectivity that may have resulted from
     * the change.
     * <p>
     * For backward compatibility, this function still allows the varray changes to be done
     * using {@link NetworkUpdate#getVarrays()}. The value specified in the parameter will 
     * override the existing varrays to maintain the same behavior. Further, only zero or one 
     * varray may be specified using this input field.  
     * 
     * @param id the URN of a ViPR network
    ...
    public NetworkRestRep updateNetwork(@PathParam("id") URI id,
       ...
       if (removedVarrays != null) network.removeAssignedVirtualArrays(StringSetUtil.uriListToSet(removedVarrays));
       if (addedVarrays != null) network.addAssignedVirtualArrays(StringSetUtil.uriListToSet(addedVarrays));
       if (removedEps != null) network.removeEndpoints(removedEps);
       if (addedEps != null) network.addEndpoints(addedEps, false);
        _dbClient.updateAndReindexObject(network);
        recordAndAudit(network, OperationTypeEnum.UPDATE_NETWORK);
     ...
     * already in the network. The endpoints can be added to the network when: <ol>
     * <li>The endpoint type is compatible with the network type. In other words WWN for FC
     * networks.</li>
     ...
     * not already in the network. The endpoints can be removed from the network when: <ol>
    ...
    private void checkNetworkExportAssociations(Network network, List<URI> varrayUris) {
        ...
        if (varrayUris == null || varrayUris.isEmpty() || network == null || 
                network.getConnectedVirtualArrays() == null || network.getConnectedVirtualArrays().isEmpty()) return;
                ...
                if ( EndpointUtility.isValidEndpoint(storagePort.getPortNetworkId(), EndpointType.SAN) ) {
                            ...
                            if (!mask.getInactive()) {
                                ...
                                for (ExportGroup group : groups) {
                                    if (!group.getInactive() && varrayUris.contains(group.getVirtualArray())) {
                                        ...
                                        throw APIException.badRequests.cannotUnassignNetworkInUse(network.getId(), group.getId(),"ExportGroup");
                                    }
                                }
                            }
                        }
                    ...
                    for (FileShare fileShare : fileShares) {
                        if (!fileShare.getInactive() && varrayUris.contains(fileShare.getVirtualArray())) {
                            ...
                            throw APIException.badRequests.cannotUnassignNetworkInUse(network.getId(),fileShare.getId(),"FileShareExport");
                        }
                    }
                }
