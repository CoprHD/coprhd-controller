...
public class SRDFDeviceController implements SRDFController, BlockOrchestrationInterface {
    ...
    private String deleteAllSrdfVolumesInCG(Map<URI, Volume> sourcesVolumeMap, final Workflow workflow,
        ...
        if (null != targetSystem) {
            // Get all targets
            addStepToRefreshSystem(DELETE_SRDF_MIRRORS_STEP_GROUP, targetSystem, targetVolumeIds, waitFor,
                    workflow);
        }
        return DELETE_SRDF_MIRRORS_STEP_GROUP;
    }
    ...
    private Method addVolumePairsToCgMethod(URI systemURI, List<URI> sourceURIs, URI remoteDirectorGroupURI,
            URI vpoolChangeUri, boolean forceAdd) {
        return new Workflow.Method(ADD_SYNC_VOLUME_PAIRS_METHOD, systemURI, sourceURIs,
                remoteDirectorGroupURI, vpoolChangeUri, forceAdd);
    }

    public boolean addVolumePairsToCgMethodStep(URI systemURI, List<URI> sourceURIs, URI remoteDirectorGroupURI,
            URI vpoolChangeUri, boolean forceAdd, String opId) {
