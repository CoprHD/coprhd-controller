    ...
    public void exportGroupRemoveVolumes(URI storageURI, URI exportGroupURI, List<URI> volumes,
        ...
        try {
            ...
            if (exportMasks != null && !exportMasks.isEmpty()) {
                ...
                generateExportGroupRemoveVolumesCleanup(workflow,
                        // If we've generated any steps, use this step group name.
                        previousStep != null ? EXPORT_GROUP_MASKING_TASK : null, 
                        storage, exportGroup, volumes, initiators);
                    ...
                    if (null != exportMaskUri) {
                        ...
                        if (exportMask.getStorageDevice().equals(storageURI)) {
                            ...
                            if (!maskInitiators.isEmpty()) {
                                maskToInitiatorsMap.put(exportMask.getId(), Lists.newArrayList(Collections2.transform(maskInitiators,
