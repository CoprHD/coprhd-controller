...
public class MetaVolumeUtils {
    ...
    public static void prepareMetaVolumes(List<Volume> volumes, long metaMemberSize, long metaMemberCount, String metaType, DbClient dbClient) {
    ...
    /**
      ...
      */
    public static MetaVolumeRecommendation getCreateRecommendation(StorageSystem storageSystem, StoragePool storagePool, long capacity, boolean isThinlyProvisioned,
        ...
        if (capabilities != null && capabilities.getIsMetaVolume()) {
            ...
            _log.info(String.format("Volume Create Recommendation (based on capabilities data): Use meta volumes: %s, Member Count: %s, Member size: %s, Meta type: %s",
        ...
        // Typically this size is 240GB, but it can be different on VMAX if auto meta configuration is enabled and minimum meta volume size is defined there
        // less than 240GB.
        ...
        if (storageSystem.getSystemType().equals(StorageSystem.Type.vmax.toString()) &&
            ...
            if (isThinlyProvisioned) { // create 8-way meta up to max possible size
                ...
                // 16 way meta for volumes from 1024 and up. After that increase meta members count as needed according to max. regular volume size limit.
                // On VMAX with auto meta configuration enabled, regular volume size limit can be small and we need to account this to make sure we do not create
    ...
    public static MetaVolumeRecommendation getExpandRecommendation(StorageSystem storageSystem, StoragePool storagePool, long capacity,
        ...
        _log.info(String.format("Create recommendation for volume expansion: Storage type: %s,  \n   capacity: %s, new capacity: %s, \n  isThinlyProvisioned: %s, " +
                       "metaMemeberSize: %s, fastExpansion: %s ", storageSystem.getSystemType(), capacity, newCapacity, isThinlyProvisioned,
        ...
        if (null != storagePool.getPoolClassName() && (storagePool.getPoolClassName().equalsIgnoreCase(StoragePool.PoolClassNames.Clar_UnifiedStoragePool.toString())
