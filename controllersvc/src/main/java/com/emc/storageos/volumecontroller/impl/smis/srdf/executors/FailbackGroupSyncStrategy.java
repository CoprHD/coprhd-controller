 ...
 */
public class FailbackGroupSyncStrategy implements ExecutorStrategy {
    private SmisCommandHelper helper;

    public FailbackGroupSyncStrategy(SmisCommandHelper helper) {
        this.helper = helper;
    }

    @Override
    public void execute(Collection<CIMObjectPath> objectPaths, StorageSystem provider) throws WBEMException {
        CIMArgument[] args = helper.getSyncPairFailBackInputArguments(objectPaths.iterator().next());
