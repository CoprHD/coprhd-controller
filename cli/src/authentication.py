...
class Authentication(object):
    ...
    def authenticate_user(self, username, password, cookiedir, cookiefile):
        ...
        try:
            if(self.__port == self.APISVC_PORT):
                ...
                if(login_response.status_code == self.SEC_REDIRECT):
                    ...
                    newHeaders[self.SEC_AUTHTOKEN_HEADER] = authToken
                    login_response = requests.get(
                    ...
                    if(login_response.status_code != requests.codes['ok']):
                        raise SOSError(
                            ...
                            login_response.text)
            elif(self.__port == self.LB_API_PORT):
                # try login via cookie first
                login_response = requests.get(
                    url, headers=self.HEADERS, verify=False,
                    cookies=cookiejar, allow_redirects=False)
                ...
                if(login_response.status_code != requests.codes['ok']):
                    # cookie doesn't work. Try real login
                    if (self.oidc_login_mode):
                        authToken = self.oidc_login(url, username, password, cookiejar)
                    else:
                ...
                raise SOSError(
                    ...
                    str(self.LB_API_PORT) + ", api service port is: " +
                    str(self.APISVC_PORT) + ".")
        ...
        print("cookie file is {}".format(form_cookiefile))
        try:
    ...
    def check_error(self, login_response):
        if (login_response.status_code != requests.codes['ok']):
            error_msg = None
    ...
    def oidc_login(self, url, username, password, cookiejar):
        ...
        location = login_response.headers['Location']
        if(login_response.status_code != 307 or not location):
            raise SOSError(SOSError.HTTP_ERR, "No authentication request returned")
        ...
        login_response = requests.get(location,
                                      ...
                                      allow_redirects=True)  # Here we want auto redirect

        self.check_error(login_response)

        return login_response.headers[self.SEC_AUTHTOKEN_HEADER]

    def direct_login(self, url, username, password, cookiejar):
        login_response = requests.get(
...
def authenticate_user(args):
    obj = Authentication(args.ip, args.port)
    obj.oidc_login_mode = args.oidc_login_mode
...
def authenticate_parser(parent_subparser, sos_ip, sos_port):
    ...
    authenticate_parser.add_argument(
        '-oidc',
        dest='oidc_login_mode',
        action='store_true',
        help='do OIDC login if appears'
    )
