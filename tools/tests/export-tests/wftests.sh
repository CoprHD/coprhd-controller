...
windows_setup() {
    if [ "${SIM}" == "1" ]; then
        ...
        WINDOWS_SIM_IP_ADDRESS=`getent hosts ${HW_SIMULATOR_IP} | awk '{print $1}'`
        grep -q -F "${WINDOWS_SIM_IP_ADDRESS} winhost1" /etc/hosts || echo "${WINDOWS_SIM_IP_ADDRESS} winhost1" >> /etc/hosts
    fi
