        ...
        public void collectMonitoringEvents() {
            ...
            try {
                ...
                if(CompatibilityStatus.COMPATIBLE.name().equalsIgnoreCase(storagesystem.getCompatibilityStatus())||
                    ...
                    events = api.queryEvents(startTimeRelative, 0).getList(); // need to use relative time for remote host (absolute time may not match on remote host)
                    ...
                    if(null != _recordableEventManager) {
                        ...
                        _logger.info("Done monitoring device {} events {}, saved to database.", storagesystem.getId(), filteredEvents.size());
                    }
                    else {
