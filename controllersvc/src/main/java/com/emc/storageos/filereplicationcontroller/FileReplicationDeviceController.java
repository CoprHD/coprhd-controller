...
public class FileReplicationDeviceController implements FileOrchestrationInterface, FileReplicationController {
    ...
    private static final String CREATE_FILE_MIRROR_PAIR_METH = "createMirrorFileSession";
    ...
    public FileStorageDevice getDevices(String Type) {
    ...
    private RemoteFileMirrorOperation getRemoteMirrorDevice() {
        return (RemoteFileMirrorOperation) devices.get(StorageSystem.Type.vmax.toString());
    }
    /**
    ...
    public String addStepsForDeleteFileSystems(Workflow workflow,
        ...
        Map<URI, FileShare> uriFileShareMap = queryFileShares(sourceDescriptors);
        ...
        for (FileShare source : uriFileShareMap.values()) {
            StringSet targets = source.getMirrorFileShares();
    ...
    protected void createMirrorFileShares(Workflow workflow, String waitFor, List<FileDescriptor> sourceDescriptors,
        ...
        for (FileDescriptor sourceDescriptor : sourceDescriptors) {
            ...
            StringSet mirrorTargets = source.getMirrorFileShares();

            for (String targetStr : mirrorTargets) {
                URI targetURI = URI.create(targetStr);
                FileShare target = uriFileShareMap.get(targetURI);
    ...
    public boolean createMirrorFileSession(final URI systemURI, final URI sourceURI,
        ...
        try {
            ...
            getRemoteMirrorDevice().doCreateMirrorLink(system, sourceURI, targetURI, completer);
    ...
    public boolean rollbackMirrorFileShareStep(URI systemURI, List<URI> sourceURIs,
        ...
        try {
            ...
            getRemoteMirrorDevice().doRollbackMirrorLink(system, sourceURIs, targetURIs, completer);
        } catch (Exception e) {
    ...
    private String deleteElementReplicaSteps(final Workflow workflow, String waitFor,
        ...
        for (FileShare source : uriFileShareMap.values()) {
            ...
            for (String mirrorTarget : mirrorTargets) {
                log.info("suspend and detach: source:{}, target:{}", source.getId(), mirrorTarget);
                ...
                if (null == target) {
                    ...
                    Workflow.Method suspendMethod = suspendMirrorLinkMethod(system.getId(),
                            source.getId(), targetURI, true);
                    String suspendStep = workflow.createStep(DELETE_FILE_MIRRORS_STEP,
                            SUSPEND_FILE_MIRRORS_STEP_DESC, waitFor, system.getId(),
                    ...
                    String detachStep = workflow.createStep(DELETE_FILE_MIRRORS_STEP,
                            DETACH_FILE_MIRRORS_STEP_DESC, suspendStep, system.getId(),
    ...
    public boolean detachMirrorFilePairStep(final URI systemURI, final URI sourceURI,
        ...
        try {
            ...
            getRemoteMirrorDevice().doDetachMirrorLink(system, sourceURI, targetURI, completer);
        } catch (Exception e) {
            ServiceError error = DeviceControllerException.errors.jobFailed(e);
            if (null != completer) {
                completer.error(dbClient, error);
    ...
    private Workflow.Method suspendMirrorLinkMethod(URI systemURI, URI sourceURI, URI targetURI, boolean consExempt) {
    ...
    public boolean cancelMirrorFilePairStep(URI systemURI, URI sourceURI, URI targetURI, boolean consExempt, String opId) {
        ...
        try {
            ...
            getRemoteMirrorDevice().doSuspendLink(system, target, completer);
        } catch (Exception e) {
            ServiceError error = DeviceControllerException.errors.jobFailed(e);
            if (null != completer) {
                completer.error(dbClient, error);
