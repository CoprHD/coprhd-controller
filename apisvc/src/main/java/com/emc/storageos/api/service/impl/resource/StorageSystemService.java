...
@DefaultPermissions(read_roles = { Role.SYSTEM_ADMIN, Role.SYSTEM_MONITOR }, write_roles = {
...
public class StorageSystemService extends TaskResourceService {
     ...
     *             When the storage system is not found.
     */
    @Override
     ...
     *             When the storage system is not registered.
     */
    ...
    /**
     * Manually create a storage system that cannot be discovered using a SMI-S
     * provider. By default the storage system will be auto-registered upon its
     * creation. For the Block type storage system, the method would add a new
     * system to the SMIS provider. The SMIS provider field in the input
     * parameter file is ignored for file type storage systems (VNX file and
     * Isilon )
     *
     * @param param
     ...
     * @return An asynchronous task corresponding to the discovery job scheduled
     *         for the new Storage System.
     *
     * @throws BadRequestException
     *             When the system type is not valid or a storage system with
     *             the same native guid already exists.
     * @throws DatabaseException
     *             When an error occurs querying the database.
     * @throws ControllerException
     *             When an error occurs discovering the storage system.
     */
    @POST
    ...
    public TaskResourceRep createStorageSystem(StorageSystemRequestParam param) throws Exception {
         ...
         * Currently we are handling monitoring for vnxfile/vmax/vnxblock/isilon
         * devices. We should not create MonitoringJob token for netapp/rp now.
         */
    ...
    /**
     * Validates SMI-S Provider attributes of the vnxFile as it is a mandatory
     * fields for indications
     * 
     * @param param
     */
    ...
    private void validateVNXFileSMISProviderMandatoryDetails(StorageSystemUpdateRequestParam param) {
         ...
         * Because while doing update client can try to update one among all
    ...
    /**
     * Validates a storage system if it already exists for same ipaddress &
     * portNumber
     * 
     * @param ipAddress
     * @param portNumber
    ...
    private void validateStorageSystemExists(String ipAddress, Integer portNumber) {
        ...
        List<StorageSystem> systems = CustomQueryUtility.getActiveStorageSystemByMgmAccessId(_dbClient,
                systemUniqueKey);
        if (systems != null && !systems.isEmpty()) {
            throw APIException.badRequests.invalidParameterProviderStorageSystemAlreadyExists(
                    "mgmtAccessPoint", systemUniqueKey);
        }
    }

    /**
     * 
     * Remove a storage system. The method would remove the storage system from
     * the system control and will remove all resources associated with the
     * storage system from the database. Note that resources (pools, ports,
     * volumes, etc.) are not removed from the storage system physically, but
...

    {
        ...
        // Ensure the storage system has no active RecoverPoint volumes under
        // management.
        if (rpHelper.containsActiveRpVolumes(id)) {
            throw APIException.badRequests.cannotDeactivateStorageSystemActiveRpVolumes();
        }

        if (DiscoveredDataObject.DataCollectionJobStatus.IN_PROGRESS.toString().equals(
                system.getDiscoveryStatus())
                || DiscoveredDataObject.DataCollectionJobStatus.SCHEDULED.toString().equals(
                        system.getDiscoveryStatus())) {
            throw APIException.serviceUnavailable
                    .cannotDeactivateStorageSystemWhileInDiscover(system.getId());
        }

        String taskId = UUID.randomUUID().toString();
        Operation op = _dbClient.createTaskOpStatus(StorageSystem.class, system.getId(), taskId,
                ResourceOperationTypeEnum.DELETE_STORAGE_SYSTEM);

        if (StringUtils.isNotBlank(system.getNativeGuid()) && system.storageSystemHasProvider()) {
            DecommissionedResource oldStorage = null;
            List<URI> oldResources = _dbClient.queryByConstraint(AlternateIdConstraint.Factory
                    .getDecommissionedResourceIDConstraint(id.toString()));
            if (oldResources != null) {
                List<DecommissionedResource> objects = _dbClient.queryObject(DecommissionedResource.class,
                        oldResources);
        ...
        PurgeRunnable.executePurging(_dbClient, _dbPurger, _asynchJobService.getExecutorService(), system,
                _retry_attempts, taskId, 60);
        return toTask(system, taskId, op);
    }

    /**
     * Allows the user to update credentials for a manually created storage
     * systems. Allows the user to update only the name field for vmax and vnx
     * block systems.
     *
     * @param id
     *            the URN of a ViPR storage system
     * @param param
     *            The storage system details to update.
     *
    ...
    @Path("/{id}")
    @CheckPermission(roles = { Role.SYSTEM_ADMIN, Role.RESTRICTED_SYSTEM_ADMIN })
    public TaskResourceRep updateStorageSystem(@PathParam("id") URI id, StorageSystemUpdateRequestParam param)
            throws ControllerException {
        StorageSystem system = _dbClient.queryObject(StorageSystem.class, id);
        ...
        if (param.getName() != null && !param.getName().isEmpty()
                && !param.getName().equalsIgnoreCase(system.getLabel())) {
            checkForDuplicateName(param.getName(), StorageSystem.class);
            system.setLabel(param.getName());
        }

        // If unlimited resources is specified and set to true, then no need to
        // look at max resources
        // If unlimited resources is set to false, then max resources should
        // also be specified. If not specified, throw error
        ...
        // if system type is vmax, vnxblock, hds, openstack or scaleio, update
        // the name or max_resources field alone.
        ...
        if (systemType.equals(StorageSystem.Type.vmax) || systemType.equals(StorageSystem.Type.vnxblock)
                || systemType.equals(StorageSystem.Type.hds)
                || systemType.equals(StorageSystem.Type.openstack)
                || systemType.equals(StorageSystem.Type.scaleio)) {
            // this check is to inform the user that he/she can not update
            // fields other than name and max_resources.
            if (param.getIpAddress() != null || param.getPortNumber() != null || param.getUserName() != null
                    || param.getPassword() != null || param.getSmisProviderIP() != null
                    || param.getSmisPortNumber() != null || param.getSmisUserName() != null
                    || param.getSmisPassword() != null || param.getSmisUseSSL() != null) {
                throw APIException.badRequests.onlyNameAndMaxResourceCanBeUpdatedForSystemWithType(systemType
                        .name());
            }
        ...
        if (((param.getIpAddress() != null && !param.getIpAddress().equals(existingIPAddress)) || (param
                .getPortNumber() != null && !param.getPortNumber().equals(existingPortNumber)))) {

            String ipAddress = (param.getIpAddress() != null) ? param.getIpAddress() : system.getIpAddress();
            Integer portNumber = (param.getPortNumber() != null) ? param.getPortNumber() : system
                    .getPortNumber();
        ...
        auditOp(OperationTypeEnum.UPDATE_STORAGE_SYSTEM, true, null, id.toString(), param.getIpAddress(),
                param.getPortNumber());
    ...
    private TaskList discoverStorageSystems(List<AsyncTask> storageTasks, StorageController controller) {
        DiscoveredObjectTaskScheduler scheduler = new DiscoveredObjectTaskScheduler(_dbClient,
                new DiscoverJobExec(controller));
     ...
     * @param id
     *            the URN of a ViPR storage system.
     * @QueryParam namespace StorageSystem Auto Discovery is grouped into
     *             multiple namespaces. Namespace is used to discover specific
     *             parts of Storage System.
     *
     *             Possible Values : UNMANAGED_VOLUMES UNMANAGED_FIESYSTEMS ALL
     *
     *             UNMANAGED_VOLUMES will discover all the Volumes which are
     *             present in the Array, and only supported on vmax and
     *             vnxblock. Using UNMANAGED_VOLUMES Namespace in other system
     *             types would result in error.
     *
     *             UNMANAGED_FILESYSTEMS will discover all the fileystems which
     *             are present in the Array, and only supported on netapp.
     *
     *             Using UNMANAGED_FILESYSTEMS Namespace in other system types
    ...
    public TaskResourceRep discoverSystem(@PathParam("id") URI id, @QueryParam("namespace") String namespace) {
        ...
        // check Storage system's compatibility for unmanaged resource
        // discovery.
        ...
        if ((Discovery_Namespaces.UNMANAGED_VOLUMES.name().equalsIgnoreCase(namespace)
                || Discovery_Namespaces.BLOCK_SNAPSHOTS.name().equalsIgnoreCase(namespace) || Discovery_Namespaces.UNMANAGED_FILESYSTEMS
                .name().equalsIgnoreCase(namespace))
                && !CompatibilityStatus.COMPATIBLE.name().equalsIgnoreCase(
    ...
    public StorageSystemRestRep registerStorageSystem(@PathParam("id") URI id) throws ControllerException {
        ...
        if (RegistrationStatus.UNREGISTERED.toString()
                .equalsIgnoreCase(storageSystem.getRegistrationStatus())) {
            storageSystem.setRegistrationStatus(RegistrationStatus.REGISTERED.toString());
            _dbClient.persistObject(storageSystem);
            startStorageSystem(storageSystem);
            auditOp(OperationTypeEnum.REGISTER_STORAGE_SYSTEM, true, null, storageSystem.getId().toString(),
                    id.toString());
        }

        // Register all Pools.
        URIQueryResultList storagePoolURIs = new URIQueryResultList();
        _dbClient.queryByConstraint(ContainmentConstraint.Factory.getStorageDeviceStoragePoolConstraint(id),
                storagePoolURIs);
        ...
        while (storagePoolIter.hasNext()) {
            ...
            if (pool.getInactive()
                    || DiscoveredDataObject.RegistrationStatus.REGISTERED.toString().equals(
        ...
        _dbClient.queryByConstraint(ContainmentConstraint.Factory.getStorageDeviceStoragePortConstraint(id),
                storagePortURIs);
        ...
        // Pool registration also update its varray relationship, so, we should
        // also update vpool to pool relation.
        ImplicitPoolMatcher.matchModifiedStoragePoolsWithAllVirtualPool(registeredPools, _dbClient,
                _coordinator);
    ...
    public StorageSystemRestRep deregisterStorageSystem(@PathParam("id") URI id) throws ControllerException {
        ...
        while (storagePoolIter.hasNext()) {
            ...
            if (pool.getInactive()
                    || DiscoveredDataObject.RegistrationStatus.UNREGISTERED.toString().equals(
                            pool.getRegistrationStatus())) {
                continue;
            }
        ...
        while (storagePortIter.hasNext()) {
            ...
            auditOp(OperationTypeEnum.DEREGISTER_STORAGE_PORT, true, null, port.getLabel(), port.getId()
                    .toString());
        }

        ImplicitPoolMatcher.matchModifiedStoragePoolsWithAllVirtualPool(modifiedPools, _dbClient,
                _coordinator);
        auditOp(OperationTypeEnum.DEREGISTER_STORAGE_SYSTEM, true, null, storageSystem.getId().toString(),
                id.toString());
    ...
    /**
     * Get information about the connectivity of the registered protection
     * system with the passed id.
     *
     * @param id
     *            the URN of a ViPR protection system.
     *
    ...
    /**
     * Return the connectivity rest response for a storage system. This method
     * uses the RPSiteArray table in Cassandra to determine which storage
     * systems are connected to this storage system via protection. In the
     * future, we may have connectivity for other features as well.
     * 
     * @param system
     *            protection system
    ...
    private StorageSystemConnectivityList getConnectivity(StorageSystem system) {
        BlockServiceApi apiImpl = BlockService.getBlockServiceImpl(system.getSystemType().toString());
        if (apiImpl == null)
     ...
     *
     * @param system
     *            Storage system to start Metering & Monitoring.
     * @throws ControllerException
     */
     ...
     * @throws ControllerException
     *             When an error occurs disconnecting the storage system.
     */
     ...
     *
     * @param systemType
     *            The type of the storage system.
     *
    ...
    public StoragePoolRestRep registerStoragePool(@PathParam("id") URI id, @PathParam("poolId") URI poolId) {
        ...
        if (RegistrationStatus.UNREGISTERED.toString().equalsIgnoreCase(pool.getRegistrationStatus())) {
            registerStoragePool(pool);
            // Pool registration also update its varray relationship, so, we
            // should also update vpool to pool relation.
            ImplicitPoolMatcher.matchModifiedStoragePoolsWithAllVirtualPool(Arrays.asList(pool), _dbClient,
                    _coordinator);
        }
    ...
    private void registerStoragePool(StoragePool pool) {
        ...
        auditOp(OperationTypeEnum.REGISTER_STORAGE_POOL, true, null, pool.getId().toString(), pool
                .getStorageDevice().toString());
    }
    ...
    /**
     * Gets all AutoTier policies associated with registered storage system with
     * the passed id. Only policies which satisfy the below will be returned 1.
     * AutoTiering should be enabled on StorageSystem 2. AutoTierPolicy should
     * be in Enabled State.
     *
     * @param id
     *            the URN of a ViPR storage system.
     * @brief List storage system autotier policies
     * @return A reference to a AutoTierPolicy List specifying the id and self
     *         link for each storage pool.
     */
    @GET
    @Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
    ...
    public AutoTierPolicyList getAllFastPolicies(@PathParam("id") URI id,
        ...
        _dbClient.queryByConstraint(ContainmentConstraint.Factory.getStorageDeviceFASTPolicyConstraint(id),
                fastPolicyURIs);
    ...
    @CheckPermission(roles = { Role.SYSTEM_ADMIN, Role.SYSTEM_MONITOR })
    public StoragePoolRestRep getStoragePool(@PathParam("id") URI id, @PathParam("poolId") URI poolId) {
        // Make sure storage system is registered.
        ArgValidator.checkFieldUriType(id, StorageSystem.class, "id");
        StorageSystem system = queryResource(id);
        ArgValidator.checkEntity(system, id, isIdEmbeddedInURL(id));
    ...
    private void registerStoragePort(StoragePort port) {
        ...
        auditOp(OperationTypeEnum.REGISTER_STORAGE_PORT, true, null, port.getId().toString(), port
                .getStorageDevice().toString());
     ...
     * @param storagePort
     */
    private void recordStoragePoolPortEvent(OperationTypeEnum opType, String description, URI resourcdId,
            String resType) {
    ...
    public StoragePortList getAllStoragePorts(@PathParam("id") URI id) {
        ...
        StorageSystem system = queryResource(id);
        ArgValidator.checkEntity(system, id, isIdEmbeddedInURL(id));
        { // Update the port metrics calculations. This makes the UI display
          // up-to-date when ports shown.
            URIQueryResultList storagePortURIs = new URIQueryResultList();
            _dbClient.queryByConstraint(
                    ContainmentConstraint.Factory.getStorageDeviceStoragePortConstraint(id), storagePortURIs);
    ...
    /**
     * Creates the storage port. It is only applicable to cinder storage systems
     * for users to manually create it on ViPR. Currently there is no API
     * available to get these information from Cinder.
     *
     * @param id
     *            the storage system id
     * @param param
     *            the StoragePortRequestParam
     * @return A StoragePortRestRep reference specifying the data for the
     *         created port.
     * @throws ControllerException
    ...
    @CheckPermission(roles = { Role.SYSTEM_ADMIN, Role.RESTRICTED_SYSTEM_ADMIN })
    @Path("/{id}/storage-ports")
    public StoragePortRestRep createStoragePort(@PathParam("id") URI id, StoragePortRequestParam param)
            throws ControllerException {
        ...
        String nativeGuid = NativeGUIDGenerator
                .generateNativeGuid(system, portName, NativeGUIDGenerator.PORT);
        ...
        StoragePortAssociationHelper.runUpdatePortAssociationsProcess(Collections.singleton(port), null,
                _dbClient, _coordinator, null);
    ...
    /**
     * Check if a storage port with the same name exists for the passed storage
     * system.
     *
     * @param name
     *            Port name
    ...
    /**
     * 
     * List all unmanaged volumes that are available for a storage
     * system.Unmanaged volumes refers to volumes which are available within
     * underlying storage systems , but still not managed in ViPR. As these
     * volumes are not managed in ViPR, there will not be any ViPR specific
    ...
    public UnManagedVolumeList getUnManagedVolumes(@PathParam("id") URI id) {
        ...
        _dbClient.queryByConstraint(
                ContainmentConstraint.Factory.getStorageDeviceUnManagedVolumeConstraint(id), result);
        while (result.iterator().hasNext()) {
            URI unManagedVolumeUri = result.iterator().next();
            unManagedVolumeList.getUnManagedVolumes().add(
                    toRelatedResource(ResourceTypeEnum.UNMANAGED_VOLUMES, unManagedVolumeUri));
        }
        return unManagedVolumeList;
    }
    ...
    /**
     * 
     * List all unmanaged file systems which are available for a storage
     * system.Unmanaged file systems refers to file systems which are available
     * within underlying storage systems , but still not managed in ViPR. As
     * these file systems are not managed in ViPR, there will not be any ViPR
     * specific details associated such as, virtual array, virtual pool, or
     * project.
     *
     * @param id
     *            the URN of a ViPR storage system
    ...
    public UnManagedFileSystemList getUnManagedFileSystems(@PathParam("id") URI id) {
        ...
        URIQueryResultList result = new URIQueryResultList();
        _dbClient.queryByConstraint(
                ContainmentConstraint.Factory.getStorageDeviceUnManagedFileSystemConstraint(id), result);
        while (result.iterator().hasNext()) {
            URI unManagedFileSystemUri = result.iterator().next();
            unManagedFileSystemList.getUnManagedFileSystem().add(
                    toRelatedResource(ResourceTypeEnum.UNMANAGED_FILESYSTEMS, unManagedFileSystemUri));
        }
        return unManagedFileSystemList;
    }
    ...
    public StorageSystemBulkRep queryBulkResourceReps(List<URI> ids) {
        Iterator<StorageSystem> _dbIterator = _dbClient.queryIterativeObjects(StorageSystem.class, ids);
    ...
    /*
     * Checks for valid Name space for discovery Valid Name space for Block ALL
     * & UNMANAGED_VOLUMES Valid Name space for File ALL & UNMANAGED_FILESYSTEMS
     */
    private boolean validateNameSpace(String nameSpace, StorageSystem storageSystem) {
        ...
        // VNXe storage system supports both block and file type unmanaged
        // objects discovery
