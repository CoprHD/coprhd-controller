...
public class VPlexMaskingWorkflowEntryPoints implements Controller {
    ...
    public void createStorageView(URI vplexURI, URI exportURI, URI exportMaskURI, Map<URI, Integer> blockObjectMap,
        ...
        try {
            _log.info(String.format("%s start", call));
            WorkflowStepCompleter.stepExecuting(stepId);
            ExportMaskCreateCompleter exportTaskCompleter = new ExportMaskCreateCompleter(exportURI, exportMaskURI, initiators,
                    blockObjectMap,
                    stepId);
            vplexExportOperationsHelper.createStorageView(vplexURI, exportURI, exportMaskURI, blockObjectMap, initiators, targets,
                    exportTaskCompleter);
            _log.info(String.format("%s end", call));
        } catch (final Exception e) {
            _log.info(call + " Encountered an exception", e);
    ...
    public void storageViewAddInitiators(URI vplexURI, URI exportURI, URI maskURI,
        ...
        try {
            _log.info(String.format("%s start", call));
            WorkflowStepCompleter.stepExecuting(stepId);
            vplexExportOperationsHelper.storageViewAddInitiators(vplexURI, exportURI, maskURI, initiatorURIs, targetURIs, sharedExportMask,
                    completer, stepId);
            _log.info(String.format("%s end", call));
