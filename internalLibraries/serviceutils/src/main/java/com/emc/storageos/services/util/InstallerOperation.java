...
public class InstallerOperation {
	...
	public static void installISOImage(Configuration config) {
        ...
        try {
            ...
            if (config.isInstallMode()) {
                ...
                cdPartition = config.getHwConfig().get(InstallerConstants.PROPERTY_KEY_DISK)+"4";
            } else {
     ...
     */
    public static String probeCpuCoreNumber() {
        final String[] cmds = {"cat", "/proc/cpuinfo"};
        Exec.Result result = Exec.sudo(CMD_DEFAULT_TIMEOUT, cmds);
        if (!result.exitedNormally() || result.getExitValue() != 0) {
     ...
     */
    public static String probeMemorySize() {
     ...
     *   ...
     */
    public static String[] getDiskAndCapacityNumber() {
	...
	public static String compareHardware(Map<String, String> hwMap, boolean checkDisk) {
		...
		hwMap.get(InstallerConstants.PROPERTY_KEY_DISK_CAPACITY);
		if (!localMemSizeStr.equals(hwMap.get(InstallerConstants.PROPERTY_KEY_MEMORY_SIZE))) {
			log.warn("Local memory {} is not the same as selected cluster {}",
					localMemSizeStr, hwMap.get(InstallerConstants.PROPERTY_KEY_MEMORY_SIZE));
			return String.format("Local memory {%s} is not the same as selected cluster {%s}",
					localMemSizeStr, hwMap.get(InstallerConstants.PROPERTY_KEY_MEMORY_SIZE));
		}
		if (!localCpuCoreStr.equals(hwMap.get(InstallerConstants.PROPERTY_KEY_CPU_CORE))) {
			log.warn("Local CPU core number {} is not the same as selected cluster {}",
					localCpuCoreStr, hwMap.get(InstallerConstants.PROPERTY_KEY_CPU_CORE));
			return String.format("Local CPU core number {%s} is not the same as selected cluster {%s}",
					localCpuCoreStr, hwMap.get(InstallerConstants.PROPERTY_KEY_CPU_CORE));
		}
		if (checkDisk && !hasSameDiskInfo(hwMap.get(InstallerConstants.PROPERTY_KEY_DISK), 
				hwMap.get(InstallerConstants.PROPERTY_KEY_DISK_CAPACITY))) {
     ...
     */
    public static String[] getOvfenvProperties() {
    ...
    public static Configuration getLocalConfiguration() {
        ...
        String[] props = getOvfenvProperties();
        Map<String, String> propMap = new HashMap<String, String>();
        for (String s : props) {
            propMap.put(s.split(InstallerConstants.PROPERTY_DELIMITER)[0], s.split(InstallerConstants.PROPERTY_DELIMITER)[1]);
        }
        ...
        conf.getHwConfig().put(InstallerConstants.PROPERTY_KEY_MEMORY_SIZE, ServerProbe.getInstance().getMemorySize());
        conf.getHwConfig().put(InstallerConstants.PROPERTY_KEY_CPU_CORE, ServerProbe.getInstance().getCpuCoreNum());

        String diskName = propMap.get(InstallerConstants.PROPERTY_KEY_DISK);
        if (diskName == null) {
            diskName = InstallerConstants.DATA_DISK_DEFAULT;
        }
        conf.getHwConfig().put(InstallerConstants.PROPERTY_KEY_DISK, diskName);
        conf.getHwConfig().put(InstallerConstants.PROPERTY_KEY_DISK_CAPACITY, ServerProbe.getInstance().getDiskCapacity(diskName));

        String netif = propMap.get(InstallerConstants.PROPERTY_KEY_NETIF);
        if (netif == null) {
            netif = InstallerConstants.NETIF_DEFAULT;
        }
        conf.getHwConfig().put(InstallerConstants.PROPERTY_KEY_NETIF, netif);
