...
mkdir -p /etc/init.d
## BEGIN:/etc/init.d/boot.getSimulatorEnv - setup ovf properties on first boot
cat > /etc/init.d/boot.getSimulatorEnv <<EOF
#!/bin/sh
#
...
#
# /etc/init.d/boot.getSimulatorEnv
#
# Boot time script to populate configuration files using parameters
# provided in ovf-env.xml
#
### BEGIN INIT INFO
# Provides:          boot.getSimulatorEnv
# Required-Start:    boot.localfs
# Required-Stop:
# Should-Stop:
# Default-Start:     3 5
# Default-Stop:      0 1 2 6
# Description:       Generate Simulator network config
### END INIT INFO
...
EOF
chmod u+x /etc/init.d/boot.getSimulatorEnv
insserv -d boot.getSimulatorEnv
## END:boot.getSimulatorEnv
...
WRKDIR=`pwd`
ls -ltr $WRKDIR/AIO_scripts/simulatorInstall.sh
