...
class VcenterClusterSettingsConverter {
    ...
    public DrsBehavior convertToDrsBehavior(String name) throws Exception {
        for(DrsBehavior drsBehavior :  DrsBehavior.values()) {
            if(drsBehavior.toString().equals(name)) return drsBehavior;
        }
    ...
    public DpmBehavior convertToDpmBehavior(String name) throws Exception {
	  	for(DpmBehavior dpmBehavior : DpmBehavior.values()) {
            if(dpmBehavior.toString().equals(name)) return dpmBehavior;
        }
    ...
    public ClusterDasAdmissionControlPolicy convertAdmissionControlPolicyToClusterDasAdmissionControlPolicy(String policy, Integer acpHostFailoverLevel, Integer acpCpuFailoverPercent, Integer acpMemoryFailoverPercent) throws Exception {
		if(policy.equals("hostFailureLevel")) {
			ClusterFailoverLevelAdmissionControlPolicy cflacp = new ClusterFailoverLevelAdmissionControlPolicy(); //  Option 1 -  Host failures cluster tolerates
			cflacp.setFailoverLevel(validateFailoverRange(acpHostFailoverLevel));
			return cflacp;
		} else if(policy.equals("resourcePercentage")) {
			ClusterFailoverResourcesAdmissionControlPolicy cdracp = new ClusterFailoverResourcesAdmissionControlPolicy(); //  Option 2 -  Percentage of cluster resources reserved as failover for spare capacity
...
//			ClusterFailoverHostAdmissionControlPolicy cfhacp = new ClusterFailoverHostAdmissionControlPolicy(); // Option 3 - Specify a failover host TODO
...
//			return cfhacp // Admission Control Policy
	...
	ClusterDasVmSettingsRestartPriority convertToClusterDasVmSettingsRestartPriority(String name) throws Exception {
	  	for(ClusterDasVmSettingsRestartPriority clusterDasVmSettingsRestartPriority : ClusterDasVmSettingsRestartPriority.values()) {
            if(clusterDasVmSettingsRestartPriority.toString().equals(name)) return clusterDasVmSettingsRestartPriority;
        }
  	...
  	ClusterDasVmSettingsIsolationResponse convertToClusterDasVmSettingsIsolationResponse(String name) throws Exception {
		for(ClusterDasVmSettingsIsolationResponse clusterDasVmSettingsIsolationResponse : ClusterDasVmSettingsIsolationResponse.values()) {
            if(clusterDasVmSettingsIsolationResponse.toString().equals(name)) return clusterDasVmSettingsIsolationResponse;
        }
   	...
   	ClusterDasConfigInfoVmMonitoringState convertToClusterDasConfigInfoVmMonitoringState(String name) throws Exception {
		for(ClusterDasConfigInfoVmMonitoringState clusterDasConfigInfoVmMonitoringState : ClusterDasConfigInfoVmMonitoringState.values()) {
            if(clusterDasConfigInfoVmMonitoringState.toString().equals(name)) return clusterDasConfigInfoVmMonitoringState;
        }
  	...
  	VirtualMachineConfigInfoSwapPlacementType convertToVirtualMachineConfigInfoSwapPlacementType(String name) throws Exception {
		for(VirtualMachineConfigInfoSwapPlacementType virtualMachineConfigInfoSwapPlacementType : VirtualMachineConfigInfoSwapPlacementType.values()) {
            if(virtualMachineConfigInfoSwapPlacementType.toString().equals(name)) return virtualMachineConfigInfoSwapPlacementType;
        }
        _log.error("Invalid swap placement " + name);
		throw new Exception("Invalid swap placement " + name);
	}

  	void convertVmMonitoringSensitivityToFinerGrainedSettings(String vmMonitoringSensitivity, ClusterVmToolsMonitoringSettings clusterVmToolsMonitoringSettings) throws Exception {
		...
		clusterVmToolsMonitoringSettings.setMaxFailures(maxFailures); // Maximum per-VM restarts - max number of failures and restarts then automated responses stop
		clusterVmToolsMonitoringSettings.setMaxFailureWindow(maxFailureWindow); // Maximum uptime - time in seconds for automated response to do its thing???
	}
}
