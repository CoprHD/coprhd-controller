...
//        }
        ...
        for (int j=1; j <= 2; j++) {
            for (int i=1; i <= 6; i++) {
                ...
                if (2*j < i) System.out.println("Expecting PlacementException due to insufficient initiators");
        ...
        for (int j=1; j <= 2; j++) {
            for (int i=1; i <= 4; i++) {
                System.out.println("*** 2 initiators across 2 networks, incremental 2 more initiators in new host: max_paths = " + i + " paths_per_initiator = " + j);
        ...
        for (int j=1; j <= 2; j++) {
            for (int i=1; i <= 6; i++) {
                System.out.println("*** 2 initiators across 2 networks, incremental 2 more initiators in same host: max_paths = " + i + " paths_per_initiator = " + j);
                testVMAX2NetAllocIncrementalAssign(net2InitiatorsMap, net2InitiatorsMapB,  i, j);
            }
        }
        ...
        for (int j=1; j <= 2; j++) {
            for (int i=1; i <= 8; i++) {
                System.out.println("*** 4 initiators across 2 networks, incremental 4 more initiators in new host: max_paths = " + i + " paths_per_initiator = " + j);
                testVMAX2NetAllocIncrementalAssign(net2InitiatorsMap, net2InitiatorsMapB,  i, j);
            }
        }
        ...
        for (int j=1; j <= 2; j++) {
            for (int i=1; i <= 12; i++) {
                System.out.println("*** 4 initiators across 2 networks, incremental 4 more initiators in same host: max_paths = " + i + " paths_per_initiator = " + j);
                testVMAX2NetAllocIncrementalAssign(net2InitiatorsMap, net2InitiatorsMapB,  i, j);
            }
        }
    ...
    public static Map<Initiator, List<StoragePort>> testAllocationAssignment(PortAllocationContext[] contexts,
        ...
        if (pathsPerInitiator > maxPaths) return assignments;
        ...
        for (int i=0; i<nnets; i++) { contexts[i].reinitialize(); }
        ...
        for (URI netURI : net2PortsNeeded.keySet()) {
            ...
            if (previousContext != null) context.copyPreviousNetworkContext(previousContext);
    ...
    static public void verifyAssignments(Map<Initiator, List<StoragePort>> assignments,
        ...
        if (existingAssignments != null) allInitiators.addAll(existingAssignments.keySet());
        ...
        for (URI host : hostInitiatorsMap.keySet()) {
            ...
            for (Initiator initiator : initiators) {
                ...
                if (portAssignments == null) continue;  // if unassigned
    ...
    static Map<URI, List<Initiator>> makeNet2InitiatorsMap(List<Initiator> initiators, int numNetworks) {
        ...
        for (int i=1; i <= numNetworks; i++) map.put(URI.create("net"+i), new ArrayList<Initiator>());
