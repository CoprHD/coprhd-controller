...
public class ConnectionManager {
	 ...
	 * 
	 * @throws Exception
	 *             When an error occurs initializing the connection manager.
	 */
	 ...
	 *            Specifies the information necessary to establish a connection.
	 * 
	 * @throws ConnectionManagerException
	 *             When a error occurs establishing the connection.
	 */
		...
		try {
			...
			if (connectionType.equals(CimConstants.CIM_CONNECTION_TYPE)) {
				...
				throw new ConnectionManagerException(
						MessageFormatter.format("Unsupported connection type {}", connectionType).getMessage());
			}
			...
			throw new ConnectionManagerException(MessageFormatter
					.format("Failed establishing a connection to the provider on host {}", hostPort).getMessage(), e);
		} finally {
	 ...
	 * 
	 * @param hostName
	 *            Specifies the host for which the CIM connection was
	 *            established.
	 * 
	 * @throws ConnectionManagerException
	 *             When a error occurs removing the connection.
	 */
	public synchronized void removeConnection(String hostName) throws ConnectionManagerException {
		...
		try {
			...
			if (!isConnected(hostName)) {
				throw new ConnectionManagerException(MessageFormatter
						.format("The connection manager is not managing a connection to host {}", hostName)
						.getMessage());
			}

			s_logger.info("Closing connection to the CIM provider on host {}", hostName);
			...
			for (CimConnection connection : _connections) {
				String connectedHostName = connection.getHost();
			...
			throw new ConnectionManagerException(MessageFormatter
					.format("Failed removing the connection to the provider on host {}", hostName).getMessage(), e);
		} finally {
			// Now resume the listener.
			_listener.resume();
		}
	...
	public synchronized boolean isConnected(String hostPort) throws ConnectionManagerException {
		...
		for (CimConnection connection : _connections) {
			String connectedHostName = generateConnectionCacheKey(connection.getHost(), connection.getPort());
			if (hostPort.equals(connectedHostName)) {
				isConnected = true;
				break;
			}
		}
	 ...
	 * 
	 * @param hostName
	 *            The name of the host on which the provider is executing.
	 * 
	 ...
	 *             When the passed host is null or blank.
	 */
	public synchronized CimConnection getConnection(String hostName) throws ConnectionManagerException {
		// Verify the passed host is not null or blank.
		if ((hostName == null) || (hostName.length() == 0)) {
			throw new ConnectionManagerException("Passed host is null or blank.");
		}
	 ...
	 * 
	 * @throws ConnectionManagerException
	 *             When an error occurs shutting don the connection manager.
	 */
	 ...
	 * 
	 * @param connectionInfo
	 *            Contains the information required to establish the connection.
	 * 
	 * @throws Exception
	 *             When an error occurs establishing the connection to the CIM
	 *             provider.
	 */
	private void createCimConnection(CimConnectionInfo connectionInfo) throws Exception {
		...
		try {
			...
			connection.connect(_configuration.getSubscriptionsIdentifier(),
					_configuration.getDeleteStaleSubscriptionsOnConnect());
			_connections.add(connection);
		} catch (Exception e) {
			throw new Exception(MessageFormatter
					.format("Failed creating connection to CIM provider on host/port {}", hostPort).getMessage(), e);
		}
	}
	 ...
	 * 
	 * @throws Exception
	 *             When an error occurs establishing the connection to the ECOM
	 *             provider.
	 */
	private void createECOMConnection(CimConnectionInfo connectionInfo) throws Exception {
		...
		try {
			...
			throw new Exception(MessageFormatter
					.format("Failed creating connection to ECOM provider on host/port {}", hostPort).getMessage(), e);
		}
	}
	 ...
	 * @throws Exception
	 *             When an error occurs establishing the connection to the ECOM
	 *             provider for the Celerra array.
	 */
	private void createCelerraConnection(CimConnectionInfo connectionInfo) throws Exception {
		...
		try {
			...
			CelerraConnection connection = new CelerraConnection(connectionInfo, _listener,
					_configuration.getIndicationFilterMap(), _configuration.getCelerraMessageSpecs());
			connection.connect(_configuration.getSubscriptionsIdentifier(),
					_configuration.getDeleteStaleSubscriptionsOnConnect());
			_connections.add(connection);
		} catch (Exception e) {
			throw new Exception(MessageFormatter
					.format("Failed creating connection to Celerra ECOM provider on host/port {}", hostPort)
					.getMessage(), e);
		}
	}
	 ...
	 * 
	 * @param cimConnection
	 *            {@link CimConnection} to make subscription for monitoring
	 * @throws Exception
	 *             Exception
	 */
	 ...
	 * 
	 * @param cimConnection
	 *            {@link CimConnection} clear subscription for the given
	 *            cimConnection
	 */
	public void unsubscribe(CimConnection cimConnection) {
		s_logger.debug("Entering {}", Thread.currentThread().getStackTrace()[1].getMethodName());
		s_logger.debug("Subscription Identifier for unsubscribe action :{}",
	 ...
	 * 
	 * @param cimConnection
	 *            {@link CimConnection} delete stale subscription for the given
	 *            cimConnection
	 */
	public void deleteStaleSubscriptions(CimConnection cimConnection) {
		s_logger.debug("Entering {}", Thread.currentThread().getStackTrace()[1].getMethodName());
		s_logger.debug("Subscription Identifier for delete subscription action :{}",
				_configuration.getSubscriptionsIdentifier());
