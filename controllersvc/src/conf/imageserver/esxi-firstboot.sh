...
if [ -n "$BOOT_VOLUME_LABEL" ]; then
    $LOG $LOG_OPT "Boot volume label to set : ${BOOT_VOLUME_LABEL}"
    CURRENT_NAME=`/bin/esxcli --formatter=csv storage filesystem list | grep VMFS | cut -d"," -f7`
#    BOOT_VOLUME_UUID=`/bin/ls -l /vmfs/volumes/datastore1 | $SED -e 's/^.*->\s*\(.*\)$/\1/'`
    $LOG $LOG_OPT "CURRENT_NAME is : ${CURRENT_NAME}"
    if [ -n "$CURRENT_NAME" ]; then
        $LOG $LOG_OPT "Assigning volume label \"${BOOT_VOLUME_LABEL}\" to /vmfs/volumes/${CURRENT_NAME}"
        COMMAND_WRAPPER "$VIMCMD hostsvc/datastore/rename  \"${CURRENT_NAME}\" \"${BOOT_VOLUME_LABEL}\"" 5 5
    fi
fi
